# #!/bin/bash
# TABLE_COUNT=`mysql -h localhost -u root -p${MARIADB_ROOT_PASSWORD} ${MARIADB_DATABASE} -e "select count(*) from information_schema.tables where table_schema='eogdatabase';\G" | grep -v "count"`
...
#   mkdir -p "/tmp/db_dump"
#   cd /tmp/db_dump
#   wget -k --content-disposition "${REMOTE_DB_DUMP_URL}"
