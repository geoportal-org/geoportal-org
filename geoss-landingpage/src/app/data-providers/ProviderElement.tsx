...
import { ProviderElementProps } from "../model/types";
...
const ProviderElement = ({ name, date, url, description, image_url, principlesStr }: ProviderElementProps) => {
        ...
        <li className="flex flex-col lg:flex-row min-h-56 w-full border border-solid lg:gap-4 my-4">
            ...
            <div className="p-4 w-full lg:w-[60%] flex flex-col items-between justify-between gap-2">
                <div className="flex w-full items-start justify-between gap-8">
                    <h2 className="text-xl text-[#061D30]">{name}</h2>
                    <button className="hover:scale-125 mt-1" onClick={() => navigator.clipboard.writeText(url)}>
                        <ShareIcon width="1.2em" height="1.2em" />
                    </button>
                </div>
                <p>Registration date: {date.substring(0, 10) || "No data"}</p>
                <Link target="_blank" className="text-[#01685B] underline" href={url}>
                    {url}
                </Link>
                <p className="line-clamp-6 lg:line-clamp-3 text-[#5C6369]">{description}</p>
                <div className="w-full flex flex-wrap gap-2">
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "discoverable")}
                        width={30}
                        height={30}
                        alt="discoverable"
                        src={discoverable}
                    ...
                    />
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "standard_encoding_using")}
                        width={30}
                        height={30}
                    ...
                    />
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "well_documented_metadata")}
                        width={30}
                        height={30}
                    ...
                    />
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "quality_documented")}
                        width={30}
                        height={30}
                    ...
                    />
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "periodically_verified")}
                        width={30}
                        height={30}
                    ...
                    />
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "reviewed_and_refreshed")}
                        width={30}
                        height={30}
                    ...
                    />
                    <TooltipImage
                        isDisabled={!principles.some((prc: string) => prc === "tagged_with_permanent_ID")}
                        width={30}
                        height={30}
