if [ -n "${UI_SERVICES_PROVIDERS}" ]; then
  ...
  awk -v r="include conf.d/cms_conf/location_services_providers.conf;" '{gsub(/###SERVICES_PROVIDERS_LOCATION###/,r)}1' /etc/nginx/conf.d/gpp.conf.old > /etc/nginx/conf.d/gpp.conf
