<template>
	<div class="bookmarks-header">
		<div class="title">
			<span v-if="mode === 'geo-likes'">
				{{$tc('geoLikes.title')}}
			</span>
			<span v-if="mode === 'bookmarks'">
				{{$tc('bookmarks.title')}}
			</span>
		...
		<div v-if="results.length" class="tools">
			<div class="check-all">
				<input v-model="checkAll" type="checkbox" id="all-bookmarks" autocomplete="off" />
				<label for="all-bookmarks">{{ $tc('bookmarks.selectAll') }}</label>
				<small>{{ `- ${numberOfChecked()} ${$tc('bookmarks.checked')}` }}</small>
			</div>
			<div class="actions">
				<span>
					<FileDownloadNotifier class="file-download-notifier" />
					<b v-show="files.length" @click="openFilesDownload();">{{$tc('bookmarks.download')}}</b>
				</span>
...
export default class HeaderComponent extends Vue {
	...
	public popupNoItemsSelected() {
		const props = {
			title: this.$tc('bookmarks.noItemsSelected'),
			subtitle: this.$tc('bookmarks.selectItems'),
			actions: [
				{event: 'accept', label: this.$tc('bookmarks.accept')},
			]
		};
	...
	public popupIncompatibleDataSources() {
		...
		const props = {
			title: this.$tc('bookmarks.incompatibleDataSources'),
			subtitle: this.$tc('bookmarks.desiredSource'),
			actions,
		};
		this.$store.dispatch(PopupActions.openPopup, {contentId: 'error', component: ErrorPopup, props});
	}
	...
	public dataSourcesCompatible() {
		const checkedOrigins: any = [];
		let compatible = true;
		for (const id of this.checkedResults) {
			for (const origin in this.resultsOrigin.byType) {
				if (this.resultsOrigin.byType[origin].includes(id.toString()) && !checkedOrigins.includes(origin)) {
	...
	public getResultsDataSource(id: string) {
	...
	public showResultsOnMap() {
		...
		window.open(`/?targetId=${targetIds}&f:dataSource=${dataSources.join()}`);
	}
	...
	public async removeBookmarks() {
		...
		for (const id of this.checkedResults) {
			...
			promises.push(GeossSearchApiService.removeBookmark(id, dataOrigin));
		}
		Promise.all(promises).then(() => this.$store.dispatch(BookmarksActions.getResults));
	}
	...
	private mounted() {
		PopupCloseService.eventBus.$on('close', ({contentId, response}: {contentId: string, response?: any}) => {
			if (this.checkedOrigins.includes(response)) { // make sure if proper error popup is closed
