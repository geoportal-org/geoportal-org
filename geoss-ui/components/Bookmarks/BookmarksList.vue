		...
		<div class="bookmark-results">
			<div v-if="resultsLoading || !results.length" class="bookmark-results__empty" :class="{loading: resultsLoading}">
				<div v-if="!resultsLoading && !results.length">
					...
					<span v-if="!resultsLoading && !results.length">{{$tc('bookmarks.yourListIsEmpty')}}</span>
				</div>
			</div>
				...
				<div class="bookmark-result__wrapper" :class="{hover: toggleShare === result.id || toggleDownloads === result.id}">
					...
					<div class="bookmark-result__content">
						<div class="bookmark-result__text-data">
							...
							<div v-if="result.contributor && result.contributor.orgName" class="bookmark-result__contributor" v-line-clamp:20="1">({{$tc('dabResult.organisation')}}: {{result.contributor.orgName}})</div>
							<div v-if="result.metadata && result.metadata.creators" class="bookmark-result__contributor" v-line-clamp:20="1">
								({{$tc('dabResult.creators')}}: <span v-for="(creator, index) of result.metadata.creators" :key="index" >{{ creator.name }}</span>)
							</div>
						...
						<div>
							<button @click="showDetails(result)" class="bookmark-result-details__more">
								<span class="">{{$tc('dabResult.seeMore')}}</span>
						...
						<div class="bookmark-result__tools">
							<div class="reviews">
								<div class="recent_views">
									...
									<span>{{ result.views || 0 }}</span> <span>{{$tc('bookmarks.recentViews')}}</span>
								</div>
							...
							<div class="actions">
								<div class="actions--main">
									<a class="show-on-map" target="_blank" :disabled="!result.hasLayer" :href="createHref(result, 'show-on-map')" :title="$tc('dabResult.showOnMap')">
										<i class="icomoon-show-on-map"></i>
									</a>
									<a v-if="addParentRefAvailable(result)" class="drill-down" target="_blank" :href="createHref(result, 'drill-down')" :title="$tc('dabResult.showInsideFolder')">
										<i class="icomoon-drill-down"></i>
									</a>
								</div>
								<div class="actions--side">
									<Share :url="prepareShareUrl(result)" />
									<a class="toggle-downloads" @click="toggleShowDownloads(result)" :class="{active: toggleDownloads === result.id}" :disabled="!result.downloads || !result.downloads.length" :title="$tc('dabResult.downloads')">
										...
										<div class="downloads-wrapper" v-show="toggleDownloads === result.id" v-if="result.downloads && result.downloads.length">
											<div>
												<span v-for="(download, index) of result.downloads" :key="index">
													...
													<a v-else @click="initCustomDownloadPopup(result, download.url)" :title="$tc('general.customDownload')">
...
export default class ListComponent extends Vue {
	...
	public zoomOnFeatures(map: any, layer: any) {
		...
		if ((layer.boundingType === 'boundingPin' || layer.boundingType === 'boundingCircle') && feature.getGeometry().getCoordinates().length === 1) {
			...
			let geometry: any = new AppVueObj.ol.geom.Polygon(coordinatesForDrawing);
			...
			if (dataOrigin === DataSources.NEXTGEOSS) {
				...
				for(const item of data) {
					...
					if(linkText) {
						...
						} else if ((extension === 'jpg' || extension === 'jpeg') && linkText.indexOf('tile') === -1 && !complex) {
							if (linkDescription) {
								downloads.push({name: linkTitle, url: linkText, desc: linkDescription, type: 'jpg', title: titleBox, score: linkScore});
							}
						} else if (extension === 'png' && linkText.indexOf('tile') === -1 && !complex) {
							if (linkDescription) {
				...
				for(const download of downloads) {
					const downloadFileSameFormatIndex: any = resultDownloads.findIndex(file => file.type === download.type);
					...
					if(downloadFileSameFormatIndex !== -1 && !resultDownloads[downloadFileSameFormatIndex].links) {
						const file: any = resultDownloads.splice(downloadFileSameFormatIndex, 1)[0];
						resultDownloads.push({
							type: download.type,
							links: [download, file]
						});
					} else if(downloadFileSameFormatIndex !== -1) {
						const downloadFileSameFormat: any = resultDownloads[downloadFileSameFormatIndex];
				...
				if(data) {
					metadata = {
						title: `<div class="d-flex flex--justify-between flex--align-center padding-right-30">
									...
									<a class="link-white" target="_blank" href="${SearchEngineService.getMetaDataUrl(result.id)}">${this.$tc('popupTitles.rawMetadata')}</a>
								</div>`,
						data
					};
				}
