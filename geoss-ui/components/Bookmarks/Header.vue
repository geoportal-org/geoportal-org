	...
	<div class="bookmarks-header">
		...
		<div v-if="results.length" class="tools">
			<div class="check-all">
				...
				<label for="all-bookmarks">{{ $t('bookmarks.selectAll') }}</label>
				<small>{{ `- ${numberOfChecked()} ${$t('bookmarks.checked')}` }}</small>
			</div>
			<div class="actions">
				<span>
					<FileDownloadNotifier class="file-download-notifier" />
					<b v-show="files.length" @click="openFilesDownload();">{{$t('bookmarks.download')}}</b>
				</span>
				<span class="show-on-map" @click="showOnMapChecked()">
					<i class="icomoon-show-on-map"></i>
					<b>{{$t('bookmarks.showOnMap')}}</b>
				</span>
				<span class="delete" @click="deleteChecked()">
					<i>✖</i>
					<b>{{$t('bookmarks.delete')}}</b>
				</span>
			</div>
		</div>
	</div>
...
export default class HeaderComponent extends Vue {
	...
	public popupNoItemsSelected() {
		const props = {
			...
			actions: [
				{event: 'accept', label: this.$t('bookmarks.accept')},
			]
		};
	...
	public getResultsDataSource(id) {
