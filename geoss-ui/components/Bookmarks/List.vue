<template>
	<div v-bar>
		<div class="bookmark-results">
			<div v-if="resultsLoading || !results.length" class="bookmark-results__empty" :class="{loading: resultsLoading}">
				<div v-if="!resultsLoading && !results.length">
					<img :src="`${staticPath()}/svg/data-gray.svg`" alt="Geoss Portal" />
					<span v-if="!resultsLoading && !results.length">{{$t('bookmarks.yourListIsEmpty')}}</span>
				</div>
			</div>
			<div v-for="result of results" :key="result.id" class="bookmark-result" :class="{'open-map': toggleMapId == result.id}" :data-id="result.id">
				<div class="bookmark-result__wrapper" :class="{hover: toggleShare === result.id || toggleDownloads === result.id}">
					<div class="bookmark-result__checkbox">
						<input type="checkbox" :id="result.id" autocomplete="off" :checked="isChecked(result.id)" @change="checkboxChange(result.id)" />
						<label :for="result.id"></label>
					</div>
					<div class="bookmark-result__image" :class="{'bookmark-result__image--default': (getImage(result.logo) !== result.logo)}">
					...
					<div class="bookmark-result__content">
						<div class="bookmark-result__text-data">
							<div class="bookmark-result__text-data--title">
								<div v-if="result.title" v-line-clamp:20="2" class="bookmark-result__title">{{result.title}}</div>
								<div v-if="result.metadata && result.metadata.title" v-line-clamp:20="2" class="bookmark-result__title">{{result.metadata.title}}</div>
								<div class="bookmark-result__text-data--datasource" :data-source="getResultsDataOrigin(result.id, result.title || result.metadata.title)"></div>
							</div>
							<div v-if="result.contributor && result.contributor.orgName" class="bookmark-result__contributor" v-line-clamp:20="1">({{$t('dabResult.organisation')}}: {{result.contributor.orgName}})</div>
							<div v-if="result.metadata && result.metadata.creators" class="bookmark-result__contributor" v-line-clamp:20="1">
								({{$t('dabResult.creators')}}: <span v-for="(creator, index) of result.metadata.creators" :key="index" >{{ creator.name }}</span>)
							</div>
							<div v-if="result.summary && typeof result.summary === 'string'" v-line-clamp:20="2" class="bookmark-result__summary" v-html-to-text="result.summary"></div>
							<div v-if="result.metadata && result.metadata.description && typeof result.metadata.description === 'string'" v-line-clamp:20="2" class="bookmark-result__summary" v-html-to-text="result.metadata.description"></div>
						</div>
						<div>
							<button @click="showDetails(result)" class="bookmark-result-details__more">
								<span class="">{{$t('dabResult.seeMore')}}</span>
						...
						<div class="bookmark-result__tools">
							<div class="reviews">
								<div class="recent_views">
									...
									<span>{{ result.views || 0 }}</span> <span>{{$t('bookmarks.recentViews')}}</span>
								</div>
								<div class="comments" v-if="result.rating" @click="setScore(result)" >
									<i v-for="score in [1, 2, 3, 4, 5]"
										:key="score"
										class="icomoon-star"
										:class="{active: Math.floor(result.rating.averageScore) >= score}"
										/>
									<span>{{ result.rating.averageScore.toFixed(1) }}</span>
								</div>
								<div class="persons" v-if="result.rating">
									<span>{{ result.rating.totalEntries }}</span>
								</div>
							</div>
							<div class="actions">
								<div class="actions--main">
									<a class="show-on-map" target="_blank" :disabled="!result.hasLayer" :href="createHref(result, 'show-on-map')" :title="$t('dabResult.showOnMap')">
										<i class="icomoon-show-on-map"></i>
									</a>
									<a v-if="addParentRefAvailable(result)" class="drill-down" target="_blank" :href="createHref(result, 'drill-down')" :title="$t('dabResult.showInsideFolder')">
										<i class="icomoon-drill-down"></i>
									</a>
								</div>
								<div class="actions--side">
									<Share :url="prepareShareUrl(result)" />
									<a class="toggle-downloads" @click="toggleShowDownloads(result)" :class="{active: toggleDownloads === result.id}" :disabled="!result.downloads || !result.downloads.length" :title="$t('dabResult.downloads')">
										<i class="icomoon-arrow down"></i>
									</a>
								</div>
								<div class="toggle-wrapper">
									<CollapseTransition>
										<div class="downloads-wrapper" v-show="toggleDownloads === result.id" v-if="result.downloads && result.downloads.length">
											<div>
												<span v-for="(download, index) of result.downloads" :key="index">
													<a v-if="download.links" @click="openDownloadLinksPopup(result, download.links)" :title="download.type.toUpperCase()">
														<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`icomoon-doc-${download.type}`"></i>
														<i v-else class="icomoon-doc-file"></i>
													</a>
													<a v-else-if="download.url.indexOf('/dhus/odata/') !== -1" @click="openSentinelLoginPopup(download.url, result)" :title="download.type.toUpperCase()">
														<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`icomoon-doc-${download.type}`"></i>
														<i v-else class="icomoon-doc-file"></i>
													</a>
													<a v-else-if="download.url.indexOf('/sdg/Series/DataCSV') !== -1" @click="getUnepFile(download.postData, result)" :title="download.type.toUpperCase()">
														<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`icomoon-doc-${download.type}`"></i>
														<i v-else class="icomoon-doc-file"></i>
													</a>
													<a @click="downloadLinkClicked(result)" :href="download.url" target="_blank" v-else-if="download.type !== 'custom-download'" :title="download.type.toUpperCase()">
														<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`icomoon-doc-${download.type}`"></i>
														<i v-else class="icomoon-doc-file"></i>
													</a>
													<a v-else @click="initCustomDownloadPopup(result, download.url)" :title="$t('general.customDownload')">
...
export default class ListComponent extends Vue {
	...
	public isChecked(id) {
		return this.$store.getters[BookmarksGetters.checkedResults].includes(id.toString());
	}

	public checkboxChange(id) {
		this.$store.dispatch(BookmarksActions.checkboxChange, id.toString());
	}

	public getResultsDataOrigin(id, name) {
	...
	public getResultsDataSource(id, name) {
		const safeId = GeossSearchApiService.safeString(id);
		const safeName = GeossSearchApiService.safeString(name);
		if (this.resultsOrigin.byId[safeId]) {
	...
	public getResultsCurrMap(id, name) {
		return 'osm';
		const safeId = GeossSearchApiService.safeString(id);
		const safeName = GeossSearchApiService.safeString(name);
		if (this.resultsOrigin.byId[safeId]) {
	...
	public createHref(result, action) {
		...
		const parentRefs = encodeURIComponent(`[{"id":"${targetId}","dataSource":"${dataSource}"}]`);
	...
	public zoomOnFeatures(map, layer) {
		const feature = layer.getSource().getFeatures()[0];
		if ((layer.boundingType === 'boundingPin' || layer.boundingType === 'boundingCircle') && feature.getGeometry().getCoordinates().length === 1) {
			let pin = feature.getGeometry().getCoordinates()[0];
			pin = ol.proj.transform(pin, 'EPSG:3857', 'EPSG:4326');
			const coordinatesForDrawing: any = MapCoordinatesUtils.coordinatesForDrawing([
				pin[0] - 10,
				Math.max(pin[1] - 10, -90),
				pin[0] + 10,
				Math.min(pin[1] + 10, 90)
			]);
			let geometry: any = new ol.geom.Polygon(coordinatesForDrawing);
	...
	public buildMaps() {
		this.$nextTick(() => {
			this.results.forEach((result, index) => {
				const currMap = this.getResultsCurrMap(result.id, result.title || result.metadata.title);
				...
				if (!this.mapsObject[result.id]) {
					this.mapsObject[result.id] = new ol.Map({
						layers,
	...
	public async setScore(result: any) {
		const dataOrigin = this.getResultsDataOrigin(result.id, result.title || result.metadata.title);
		const [, comments] = await to(GeossSearchApiService.getComments(result.id, dataOrigin));
		const props = {
			id: result.id,
		...
		this.$store.dispatch(PopupActions.openPopup, {contentId: 'rating', title: this.$t('popupTitles.rateResouce'), component: DabResultRating, props});
	}

	public prepareShareUrl(result) {
		return this.createHref(result, 'share');
	}

	public toggleShowDownloads(result) {
		this.toggleShare = 0;
	...
	public initResultDownloads(results) {
		for (const result of results) {
			...
			if (dataOrigin === DataSources.NEXTGEOSS) {
				...
				if (result.files) {
					...
					downloads.push({name: result.metadata.title, url: result.links.doi, desc: result.metadata.description, type: 'html'});
				}
				resultDownloads = downloads;
			} else {
				...
				for(const item of data) {
					...
					if(linkText) {
						...
						for(const category of UtilsService.getArrayByString(result, 'category._attributes')) {
						...
						if (protocol === 'gwp_un_sd_/v1/sdg') {
							...
							if (wordsArray.length === 3 && wordsArray[0] === 'SDG' && wordsArray[2] === 'Indicator') {
								...
								timeSeriesArray.push({id: seriesId, text: seriesDesc});
								const postData = {indicator: wordsArray[1], series: [seriesId]};
								downloads.push({name: `SDG-${wordsArray[1]}-${seriesId}.csv`, url: 'https://unstats.un.org/SDGAPI/v1/sdg/Series/DataCSV', postData, desc: `${wordsArray[1]}, ${seriesDesc}`, type: 'csv'});
							}
						}
					}
				...
				for(const download of downloads) {
					const downloadFileSameFormatIndex = resultDownloads.findIndex(file => file.type === download.type);
					...
					if(downloadFileSameFormatIndex !== -1 && !resultDownloads[downloadFileSameFormatIndex].links) {
						const file = resultDownloads.splice(downloadFileSameFormatIndex, 1)[0];
						resultDownloads.push({
							type: download.type,
							links: [download, file]
						});
					} else if(downloadFileSameFormatIndex !== -1) {
						const downloadFileSameFormat = resultDownloads[downloadFileSameFormatIndex];
	...
	public getDownloadLinkStatus(score) {
		let scoreText;
		let scoreClass;

		if (score >= 0 && score < 20) {
			scoreText = `${this.$t('fileDownloadsPopup.veryUnreliable')} (${this.$t('fileDownloadsPopup.score')} ${score}%`;
			scoreClass = 'av-lowest';
		} else if (score >= 20 && score < 40) {
			scoreText = `${this.$t('fileDownloadsPopup.frequentlyUnavailable')} (${this.$t('fileDownloadsPopup.score')} ${score}%`;
			scoreClass = 'av-low';
		} else if (score >= 40 && score < 60) {
			scoreText = `${this.$t('fileDownloadsPopup.sometimesUnavailable')} (${this.$t('fileDownloadsPopup.score')} ${score}%`;
			scoreClass = 'av-med';
		} else if (score >= 60 && score < 80) {
			scoreText = `${this.$t('fileDownloadsPopup.mostlyAvailable')} (${this.$t('fileDownloadsPopup.score')} ${score}%`;
			scoreClass = 'av-high';
		} else if (score >= 80 && score <= 100) {
			scoreText = `${this.$t('fileDownloadsPopup.veryReliable')} (${this.$t('fileDownloadsPopup.score')} ${score}%`;
			scoreClass = 'av-highest';
		} else {
			scoreText = `${this.$t('fileDownloadsPopup.noInfo')}`;
	...
	public openDownloadLinksPopup(result, links) {
		const props = {
			result,
			links
		};

		this.$store.dispatch(PopupActions.openPopup, {contentId: 'download-links', title: this.$t('popupTitles.downloadLinks'), component: DabResultDownloads, props}).then((sentinelUrl: string) => {
			if(sentinelUrl) {
	...
	public openSentinelLoginPopup(url: string, result) {
		if (!Liferay.ThemeDisplay.isSignedIn() || !this.$store.getters[MyWorkspaceGetters.settings].dhusUsername) {
			const props = {
				url,
				result
			};

			this.$store.dispatch(PopupActions.openPopup, {contentId: 'sentinel-login', title: this.$t('popupTitles.sentinelDataAccess'), component: SentinelLogin, props});
		} else {
			window.open(SearchEngineService.getDhusProxyUrl(url));
		}
	}

	public async getUnepFile(params, result) {
		...
		LogService.logElementClick(null, null, result.id, null, 'Direct download', null, result.contributor.orgName, result.title);
	}

	public downloadLinkClicked(result) {
	...
	public async initCustomDownloadPopup(result, baseUrl) {
		...
		if(!customDownloadOptions) {
			if (baseUrl.includes('service=WCS&version=2.0.1')) {
				...
				for(const item of options) {
					let outputSizeVal: any | string[] = UtilsService.getArrayByString(
						item,
		...
		if (baseUrl.includes('service=WCS&version=2.0.1')) {
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'custom-download', title: this.$t('popupTitles.customizeDownload'), component: CustomDownloadWCS, props});
		} else {
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'custom-download', title: this.$t('popupTitles.customizeDownload'), component: DabResultCustomDownload, props});
		}
	}

	public async showDetails(result) {
		const dataSource = this.getResultsDataSource(result.id, '');
		const title = dataSource === 'zenodo' ? UtilsService.getPropByString(result, 'metadata.title') : UtilsService.getPropByString(result, 'title');
		let metadata = null;

		LogService.logElementClick(null, null, result.id, null, 'Show details', null, null, title);
		...
		if(!metadata) {
			if (dataSource === DataSources.ZENODO) {
				isSatellite = false;
				metadata = {
					title: this.$t('popupTitles.resourceDetails'),
					data: {
						...result,
						acquisition: {
							platform: 'ZENODO'
			...
			} else if((result.acquisition && Object.keys(result.acquisition.platform).length)) {
				...
				const [, data] = await to(GeossSearchApiService.getDabResultMetadata(result.id));
				if(data) {
					metadata = {
						title: `<div class="d-flex flex--justify-between flex--align-center padding-right-30">
									<span>${this.$t('popupTitles.resourceDetails')}</span>
									<a class="link-white" target="_blank" href="${SearchEngineService.getMetaDataUrl(result.id)}">${this.$t('popupTitles.rawMetadata')}</a>
								</div>`,
						data
					};
				}
		...
		if(metadata) {
			...
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'metadata', title: metadata.title, component: DabResultMetadata, props});
		}
	}
...
.bookmark-result {
	...
	&__wrapper {
		...
		&.hover {
			...
			.bookmark-result__tools {
				background: -moz-linear-gradient(top,  rgba(255,255,255,0.25) 0%, rgba(255,255,255,1) 12%); /* FF3.6-15 */
				background: -webkit-linear-gradient(top,  rgba(255,255,255,0.25) 0%,rgba(255,255,255,1) 12%); /* Chrome10-25,Safari5.1-6 */
				background: linear-gradient(to bottom,  rgba(255,255,255,0.25) 0%,rgba(255,255,255,1) 12%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
	...
	&__tools {
		...
		.reviews {
			...
			.recent_views {
				&:before {
					background: url('#{$staticPath}/img/review.svg') center center no-repeat;
				}
				span {
					&:nth-child(2) {
						margin-right: 3px;
			...
			.comments {
				...
				&:before {
					background: url('#{$staticPath}/img/rating-comment.svg') center center no-repeat;
				}
				span {
					margin-left: 3px;
				}
			...
			.persons {
				...
				&:before {
					background: url('#{$staticPath}/img/person.svg') center center no-repeat;
					width: 16px;
					height: 16px;
					margin-bottom: 3px;
					display: block;
	...
	&__text-data {
		...
		&--datasource {
			...
			&[data-source='dab'] {
				background-image: url('#{$staticPath}/img/logo_dab.png');
			}

			&[data-source='geoss_cr'] {
				background-image: url('#{$staticPath}/img/logo_geoss_cr.png');
			}
			
			&[data-source='amerigeoss_ckan'] {
				background-image: url('#{$staticPath}/img/logo_amerigeoss_ckan.png');
			}

			&[data-source='next_geoss'] {
				background-image: url('#{$staticPath}/img/logo_geoss_cr.svg');
			}

			&[data-source='zenodo'] {
				background-image: url('#{$staticPath}/img/logo_zenodo.png');
			}
			
			&[data-source='wikipedia'] {
				background-image: url('#{$staticPath}/img/logo_wikipedia.png');
			}
		}
