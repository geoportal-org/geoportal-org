<template>
	<button v-if="isBulkDownloadEnabled" class="disabled-transparent" :class="{pulsate}" @click="openBulkDownloadPopup()" :disabled="disabled" v-show="links.length || files.length">
...
export default class BulkDownloadNotifierComponent extends Vue {
	...
	@Watch('openTrigger')
	private onOpenChanged(val, oldVal) {
		if (val) {
