		...
		<div class="bulk-download">
			<div v-if="links.length">
				...
				<div class="bulk-download__buttons text-center">
					<button class="red-btn-contour" @click="clearAll()">{{$t('fileDownloadsPopup.clearAll')}}</button>
					<button class="green-btn-default" @click="preparePackage()" :disabled="downloadButtonDisabled()">{{$t('fileDownloadsPopup.preparePackage')}}</button>
				</div>
			</div>
			<div v-else class="bulk-download__no-links">
				<span>{{$t('fileDownloadsPopup.listIsEmpty')}}</span>
			</div>
		</div>
...
export default class BulkDownloadPopupComponent extends Vue {
	...
	public preparePackage() {
		...
		this.links.forEach(link => {
			if (this.selectedLinks.findIndex(item => item.url === link.url) !== -1) {
	...
	public removeLink(url) {
		this.$store.dispatch(BulkDownloadActions.removeLink, url);
		this.selectedLinks = this.selectedLinks.filter(item => item.url !== url);
		if (!this.links.length && !this.files.length) {
			PopupCloseService.closePopup('bulk-download');
		}
	}
