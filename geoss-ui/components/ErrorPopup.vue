<template>
	<div class="error-popup popup-default">
		<div class="popup-default__title">
			...
			<div>{{title}}</div>
			<button @click="close()" class="close-btn" :title="$t('general.close')" :aria-label="$t('general.close')"></button>
		</div>
		...
		<div class="popup-default__actions">
			...
			<button v-if="(!actions || !actions.length) && description" class="blue-btn-default" @click="toggleDescription()">{{showDetails ? $t('general.hideDetails') : $t('general.showDetails')}}</button>
