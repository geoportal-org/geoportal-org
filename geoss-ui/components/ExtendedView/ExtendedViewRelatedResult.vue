<template>
	<div class="ev-related-result">
		<div v-if="result.id === 'placeholder'" class="ev-related-result__placeholder">
			<img :src="`${staticPath()}/svg/${result.type}-gray.svg`" :alt="result.type" />
		</div>
		<div v-else class="ev-related-result__wrapper">
			<div class="ev-related-result__image" :class="{'ev-related-result__image--default': (getImage(result.logo) !== result.logo)}">
			...
			<div class="ev-related-result__text-data" @click="changeMainResult()">
				...
				<div v-if="summary && typeof summary === 'string'" v-line-clamp:20="2" class="ev-related-result__summary" v-html-to-text="summary"></div>
			</div>
		</div>
	</div>
</template>
