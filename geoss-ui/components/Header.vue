<template>
    <header class="header" :class="{ active: menuOpened }">
        <div class="header__left">
            <button class="header__menu-trigger" :aria-label="$t('menu.toggle')" @click="toggleMenu()"
            ...
            <a title="GEOSec" href="http://www.earthobservations.org/index.php" target="_blank"
                ...
                <img :src="`${staticPath()}/img/geoss-logo-white.png`" alt="GEO Secretary logo" />
            </a>
        </div>
        ...
        <div class="header__right">
            <a title="CNR_IIA" href="https://en.iia.cnr.it/" target="_blank" data-tutorial-tag="header-logo-cnr">
                <img :src="`${staticPath()}/img/CNR_logo_IIA-1-white.png`" alt="CNR IIA logo" />
            </a>
            <a title="ESA" href="http://www.esa.int" target="_blank" data-tutorial-tag="header-logo-esa">
                <img :src="`${staticPath()}/img/ESA-logo-51-white.png`" alt="ESA logo" />
            </a>
            <div class="header__language-switcher" v-click-outside="closeLangContainer"
                data-tutorial-tag="header-language-switch">
                <button @click="toggleLangContainer()" :title="$t('menu.languageToggle')"
                    :aria-label="$t('menu.languageToggle')">
...
import { MenuActions } from '@/stores/menu/menu-actions';
import { MenuGetters } from '@/stores/menu/menu-getters';
...
export default class HeaderComponent extends Vue {
    ...
    public changeLangLocale(locale: string) {
        if (this.langLocale !== locale) {
            ...
            this.$cookies.set('GUEST_LANGUAGE_ID', locale, { expires: nextYear })
    ...
    private mounted() {
        if (this.$cookies.get('GUEST_LANGUAGE_ID')) {
            this.$cookies.set('GUEST_LANGUAGE_ID', this.$cookies.get('GUEST_LANGUAGE_ID'));
            this.$store.dispatch(GeneralActions.setLangLocale, this.$cookies.get('GUEST_LANGUAGE_ID'));

        } else {
            this.$cookies.set('GUEST_LANGUAGE_ID', 'en_US');
