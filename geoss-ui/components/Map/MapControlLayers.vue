<template>
	<div class="map-control-layers">
		<button class="map-control-layers__trigger disabled-transparent" @click="toggleBox()" :title="$t('mapControls.layers')"
		...
		<div class="map-control-layers__wrapper" :class="{active: showBox}" v-if="mapLayers.length">
			<div class="map-control-layers__inner-wrapper">
				<div class="map-control-layers__title">
					<i class="icomoon-layers"></i>
					{{$t('map.layersHandling')}}
					<button
						v-if="!isWidget"
						class="compare-mode-button disabled-transparent"
						@click="compareLayerClick()"
						:disabled="comparisonModeDisallowed && !comparisonModeEnabled"
						:title="$t('map.compareLayer')"
						:class="{active: comparisonModeEnabled}"
				...
				<div class="map-control-layers__layers" v-bar>
					<div :class="{'compare-mode': comparisonModeEnabled}">
						<div class="map-control-layers__layer" v-for="layerData in mapLayers" :key="layerData.id" v-if="(comparisonModeEnabled && layerData.type !== LayerTypes.BOUNDING && layerData.visible) || !comparisonModeEnabled">
							<div class="d-flex flex--align-center flex--justify-between">
								<div class="d-flex flex--align-center full-width">
									<span class="map-control-layers__layer-img bounding" v-if="layerData.type === LayerTypes.BOUNDING"></span>
									<span class="map-control-layers__layer-img area-of-interest" v-if="layerData.type === LayerTypes.SELECTEDAREA"></span>
									<div class="map-control-layers__layer-img preview" v-if="layerData.type !== LayerTypes.BOUNDING && layerData.type !== LayerTypes.SELECTEDAREA">
										<span v-if="!layerData.image">{{$t('map.previewUnavailable')}}</span>
										<img v-else :src="layerData.image" />
									</div>
									<div class="checkbox" v-if="!comparisonModeEnabled" data-tutorial-tag="layers-handling-visibility">
										<input type="checkbox" :id="layerData.id" :checked="layerData.visible" @change="changeLayerVisibility($event, layerData)" />
										<label :for="layerData.id">
											<i class="icomoon-eye"></i>
											<span>{{(layerData.type === LayerTypes.SELECTEDAREA ? $t('mapControls.areaOfInterest') : layerData.title)}}</span>
										</label>
									</div>
									<div class="checkbox" v-if="comparisonModeEnabled" :class="{disabled: comparedLayersIds.length >= 2 && comparedLayersIds.indexOf(layerData.id) === -1}" data-tutorial-tag="layers-handling-comparison">
										<input type="checkbox" :id="'compare-layer-' + layerData.id" :disabled="comparedLayersIds.length >= 2 && comparedLayersIds.indexOf(layerData.id) === -1" @change="setLayerComparisonState($event, layerData)" />
								...
								<div class="map-control-layers__actions" v-if="!comparisonModeEnabled">
									...
									<button class="map-control-layers__legend"
										...
										@click="toggleLegend(layerData)"
										data-tutorial-tag="layers-handling-legend">
										<i class="icomoon-legend"></i>
									</button>
									<button class="map-control-layers__remove cross"
										v-if="layerData.id !== LayerTypes.BOUNDING"
										:title="$t('map.removeLayer')"
										@click="removeLayer(layerData.id)"
										data-tutorial-tag="layers-handling-remove">
									</button>
								</div>
							</div>
							<div v-if="!comparisonModeEnabled">
								<CollapseTransition>
									<div class="map-control-layers__transparency-control" v-show="activeSettingsLayerId === layerData.id">
										...
										<vue-slider :ref="`layer-transparency-slider-${layerData.id}`" :real-time="true" 
											:use-keyboard="true"
											tooltip="always"
											class="transparency-control-slider"
...
export default class MapControlLayersComponent extends Vue {
	...
	public comparedLayersIds = [];
	private mapElement: HTMLElement;
	private map;
	private comparisonBar: HTMLDivElement;
	private comparisonBarVerticalLine: HTMLDivElement;

	private comparisonBarPressed = false;

	private compareLayersList = {};
	private compareLayerZIndex;
	...
	public compareLayerClick() {
		const wmsMapLayers = this.wmsMapLayers.filter(layer => layer.visible);
		...
		if (this.comparisonModeEnabled) {
			this.compareLayerZIndex = wmsMapLayers.map(layer => layer.value.getZIndex()).reduce((p, c) => p < c ? c : p, 0) + 1;
			wmsMapLayers.forEach(layer => this.addLayerToComparisonList(layer));
		}
		this.activeSettingsLayerId = null;
	...
	private onActiveCompareLayerChange(newLayer, oldLayer) {
		if(oldLayer) {
	...
	@Watch('wmsMapLayers')
	private onWmsMapLayersChange(val, oldVal) {
	...
	private comparisonBarMoveEventHandler(event) {
