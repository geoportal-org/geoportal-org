<template>
    <div class="map-control-layers">
        <button class="map-control-layers__trigger disabled-transparent" @click="toggleBox()"
            :title="$tc('mapControls.layers')" :disabled="(!boundingLayers.length && mapLayers.length < 2) ||
                isExtendedViewActive
                " v-click-outside="{
                fn: closeBox,
                excludeSelectors:
                    '.map-control-layers__inner-wrapper, .tutorial-tag, .tutorial-mode, .tutorial-off'
            }">
        ...
        <div class="map-control-layers__wrapper" :class="{ active: showBox }" v-if="mapLayers.length">
            <div class="map-control-layers__inner-wrapper">
                <div class="map-control-layers__title">
                    ...
                    <button v-if="!isWidget" class="compare-mode-button disabled-transparent"
                        @click="compareLayerClick()" :disabled="comparisonModeDisallowed && !comparisonModeEnabled
                            " :title="$tc('map.compareLayer')" :class="{ active: comparisonModeEnabled }"
                        data-tutorial-tag="layers-handling-compare-mode">
                ...
                <div class="map-control-layers__layers" v-bar>
                    <div :class="{ 'compare-mode': comparisonModeEnabled }">
                        <div class="map-control-layers__layer" v-for="layerData in mapLayers" :key="layerData.id" v-if="
                        ...
                        ">
                            <div class="d-flex flex--align-center flex--justify-between">
                                <div class="d-flex flex--align-center full-width">
                                    <span class="map-control-layers__layer-img bounding" v-if="
                                        layerData.type ===
                                        LayerTypes.BOUNDING
                                    "></span>
                                    <span class="map-control-layers__layer-img area-of-interest" v-if="
                                        layerData.type ===
                                        LayerTypes.SELECTEDAREA
                                    "></span>
                                    <div class="map-control-layers__layer-img preview" v-if="
                                        layerData.type !==
                                        LayerTypes.BOUNDING &&
                                        layerData.type !==
                                        LayerTypes.SELECTEDAREA
                                    ">
                                        <span v-if="!layerData.image">{{
                                            $tc('map.previewUnavailable')
                                            }}</span>
                                        <img v-else :src="layerData.image" />
                                    </div>
                                    <div class="checkbox" v-if="!comparisonModeEnabled"
                                        data-tutorial-tag="layers-handling-visibility">
                                        <input type="checkbox" :id="layerData.id" :checked="layerData.visible" @change="
                                        ...
                                        <label :for="layerData.id">
                                            ...
                                            <span>{{
                                                layerData.type ===
                                                    LayerTypes.SELECTEDAREA
                                                    ? $tc(
                                                        'mapControls.areaOfInterest'
                                                    )
                                                    : layerData.title
                                            }}</span>
                                        </label>
                                    </div>
                                    <div class="checkbox" v-if="comparisonModeEnabled" :class="{
                                        disabled:
                                            comparedLayersIds.length >= 2 &&
                                            comparedLayersIds.indexOf(
                                                layerData.id
                                            ) === -1
                                    }" data-tutorial-tag="layers-handling-comparison">
                                        <input type="checkbox" :id="'compare-layer-' + layerData.id
                                            " :disabled="comparedLayersIds.length >= 2 &&
                                                comparedLayersIds.indexOf(
                                                    layerData.id
                                                ) === -1
                                                " @change="
                                                setLayerComparisonState(
                                                    $event,
                                                    layerData
                                                )
                                                " />
                                ...
                                <div class="map-control-layers__actions" v-if="!comparisonModeEnabled">
                                    <button class="map-control-layers__center-map" @click="centerMap(layerData)"
                                        :title="$tc('map.centerMapOnLayer')" v-if="
                                            (layerData.id !==
                                                LayerTypes.BOUNDING &&
                                    ...
                                    <button class="map-control-layers__settings" @click="openLayerSettings(layerData)"
                                        :title="$tc('map.layerSettings')" :class="{
                                            active:
                                    ...
                                    <button class="map-control-layers__legend" :title="$tc('map.legend')" :class="{
                                        active:
                                            activeLayerLegend &&
                                            activeLayerLegend.id ===
                                            layerData.id
                                    }" v-if="
                                            layerData.legend &&
                                            supportedLegendTypes.includes(
                                                layerData.legend.type
                                            )
                                        " @click="toggleLegend(layerData)" data-tutorial-tag="layers-handling-legend">
                                        <i class="icomoon-legend"></i>
                                    </button>
                                    <button class="map-control-layers__remove cross" v-if="
                                        layerData.id !== LayerTypes.BOUNDING
                                    " :title="$tc('map.removeLayer')" @click="removeLayer(layerData.id)"
                                        data-tutorial-tag="layers-handling-remove"></button>
                                </div>
                            </div>
                            <div v-if="!comparisonModeEnabled">
                                <CollapseTransition>
                                    <div class="map-control-layers__transparency-control" v-show="activeSettingsLayerId ===
                                        layerData.id
                                        ">
...
export default class MapControlLayersComponent extends Vue {
    ...
    get mapLayers() {
        ...
        return this.$store.getters[MapGetters.layers].filter(
            (layerData: LayerData) =>
    ...
    get wmsMapLayers() {
        ...
        return this.$store.getters[MapGetters.layers].filter(
            (layerData: LayerData) =>
                layerData.type.indexOf(LayerTypes.BOUNDING) === -1 &&
                layerData.type !== LayerTypes.SELECTEDAREA
        )
    }
    ...
    public compareLayerClick() {
        const wmsMapLayers = this.wmsMapLayers.filter(
            (layer: { visible: any }) => layer.visible
        )
        ...
        if (this.comparisonModeEnabled) {
            this.compareLayerZIndex =
                wmsMapLayers
                    .map((layer: { value: { getZIndex: () => any } }) =>
                        layer.value.getZIndex()
                    )
                    .reduce((p: number, c: number) => (p < c ? c : p), 0) + 1
            wmsMapLayers.forEach((layer: any) =>
                this.addLayerToComparisonList(layer)
            )
        }
        this.activeSettingsLayerId = null
        ...
        if (!this.comparisonModeEnabled) {
            Object.keys(this.compareLayersList).forEach((item) =>
                this.compareLayersList[item].layer.value.setZIndex(
                    this.compareLayersList[item].defaultZIndex
                )
            )
    ...
    public removeLayer(layerDataId: string) {
        ...
        if (
            this.activeLayerLegend &&
            this.activeLayerLegend.id === layerDataId
        ) {
            this.$store.dispatch(
                MapActions.setActiveLayerLegend,
                this.getTopmostActiveLayer()
            )
        }
        const wmsMapLayers = this.wmsMapLayers.filter(
            (layer: { visible: any }) => layer.visible
        ...
        this.comparisonModeDisallowed = wmsMapLayers.length < 2
        if (
            wmsMapLayers.length < 2 ||
            this.activeCompareLayerId === layerDataId
        ) {
            this.$store.dispatch(MapActions.setActiveCompareLayer, null)
        }
    }
    ...
    public changeLayerTransparency(layerData: LayerData, value: number) {
        this.$store.dispatch(MapActions.changeLayerTransparency, {
            id: layerData.id,
            value
        })
    }
    ...
    public changeLayerVisibility(event: Event, layerData: LayerData) {
        ...
        if (
            layerData.legend &&
            this.supportedLegendTypes.includes(layerData.legend.type)
        ) {
            ...
            if (
                !target.checked &&
                this.activeLayerLegend &&
                this.activeLayerLegend.id === layerData.id
            ) {
                this.$store.dispatch(
                    MapActions.setActiveLayerLegend,
                    this.getTopmostActiveLayer()
        ...
        if (
            wmsMapLayers.length < 2 ||
            this.activeCompareLayerId === layerData.id
        ) {
            this.$store.dispatch(MapActions.setActiveCompareLayer, null)
        }
    }
    ...
    public setLayerComparisonState(event: Event, layerData: LayerData) {
        ...
        if (!target.checked) {
            layerData.value.setZIndex(
                this.compareLayersList[layerData.id].defaultZIndex
            )
        }
        this.comparedLayersIds = Object.keys(this.compareLayersList).filter(
            (key) => this.compareLayersList[key].checked
        )
        const comparisonLayersKeys = Object.keys(this.compareLayersList)
        const checkedLayersCount = comparisonLayersKeys.filter(
            (key) => this.compareLayersList[key].checked
        ).length
        if (checkedLayersCount > 1) {
            comparisonLayersKeys.forEach(
                (key) =>
                    key !== layerData.id &&
                    this.compareLayersList[key].layer.value.setZIndex(
                        this.compareLayersList[key].defaultZIndex
                    )
            )
            this.$store.dispatch(
                MapActions.setActiveCompareLayer,
                this.compareLayersList[layerData.id].layer
            )
            this.activeCompareLayerId =
                this.compareLayersList[layerData.id].layer.id
            ...
            if (
                this.activeCompareLayer &&
                this.activeCompareLayer !== layerData
            ) {
                const defaultLayerZIndex =
                    this.compareLayersList[this.activeCompareLayer.id]
                        .defaultZIndex
    ...
    private onActiveCompareLayerChange(
        newLayer: {
            value: {
                on: (
                    arg0: string,
        ...
        if (oldLayer) {
            oldLayer.value.un(
                'precompose',
                this.handleActiveComparisonLayerPrecomposeEvent
            )
    ...
    private handleActiveComparisonLayerPrecomposeEvent(event: {
        context: any
    }) {
        ...
        this.$store.dispatch(
            MapActions.setCompareBarPosition,
            this.getBarPosition()
        )
        ctx.save()
        ctx.beginPath()
    ...
    private getBarPosition(): number {
        ...
        comparisonBarOffsetLeft +=
            this.comparisonBarVerticalLine!.offsetLeft +
            this.comparisonBarVerticalLine!.offsetWidth / 2
    ...
    private getTopmostActiveLayer() {
        ...
        for (let i = lastIndex; i >= 0; i--) {
            if (
                this.wmsMapLayers[i].legend &&
                this.supportedLegendTypes.includes(
                    this.wmsMapLayers[i].legend.type
                ) &&
                this.wmsMapLayers[i].visible
            ) {
                if (
                    !this.activeLayerLegend ||
                    this.wmsMapLayers[i].id !== this.activeLayerLegend.id
                ) {
    ...
    private mounted() {
        if (!this.isWidget) {
            ...
            this.comparisonBarVerticalLine =
                this.comparisonBar!.querySelector('.vertical-line')
            ...
            this.comparisonBar!.addEventListener('mousedown', (event) => {
                ...
                document.addEventListener(
                    'mousemove',
                    this.comparisonBarMoveEventHandler
                )
            })
    ...
    private comparisonBarMoveEventHandler(event: TouchEvent | any) {
        ...
        if (event.targetTouches && event.targetTouches.length) {
            offsetX = calculateOffset(
                event.targetTouches[0].clientX,
                this.mapElement!.clientWidth
            )
        } else {
...
.map-control-layers {
    ...
    &__wrapper {
        ...
        right: calc(100% + 20px);
        width: 0;
        transition: right 0s 0.5s linear, left 0s 0.5s linear,
            width 0s 0.5s linear;
        ...
        &.active {
            ...
            .map-control-layers__inner-wrapper {
                ...
                margin-left: 0;
                opacity: 1;
                transition: width 0.5s 100ms ease-in-out,
                    margin-left 0.5s 100ms ease-in-out, opacity 0.5s ease-in-out;

                @media (max-width: $breakpoint-lg) {
                    transform: translateY(0%);
                    transition: transform 0.5s 100ms ease-in-out,
