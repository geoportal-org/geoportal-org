<template>
	<div v-show="enable" :class="`map-progress ${percentage == 100 ? 'complete' : 'loading'}`" :style="`width: ${this.percentage}%;`"></div>
</template>
...
export default class MapProgressBarComponent extends Vue {
	...
	private onProgressBarPercentage(newValue) {
		this.percentage = newValue;
	}

	@Watch('progressBarEnable')
	private onProgressBarEnable(newValue) {
