<template>
	<div class="popup" :class="{visible: visible, 'on-top': (currentQueueItem && currentQueueItem.contentId === 'dab-request-too-long')}">
...
export default class PopupComponent extends Vue {
	...
	private onQueueChanged() {
		if(this.queue.length && JSON.stringify(this.queue[0]) !== JSON.stringify(this.currentQueueItem)) {
			...
			TutorialTagsService.refreshTagsGroup('popup-', true, Timers.openPopup);
	...
	private mounted() {
		PopupCloseService.eventBus.$on('close', ({contentId, response}: {contentId: string, response?: any}) => {
			if(this.currentQueueItem && this.currentQueueItem.contentId === contentId) {
				...
				if(response) {
					this.$store.dispatch(PopupActions.closePopupWithResponse, {contentId, response});
				} else {
