    ...
    <div v-if="crRelationSrc && crRelationSrc.id && crRelationSrc !== ''" class="crRelationSrc">
        <div class="crRelationSrc__top">
            <div class="crRelationSrc__title">
                <button @click="cancel()" :title="$tc('popupContent.cancel')"></button>
            ...
            <div class="d-flex flex--align-center flex--justify-end flex--1">
                <button class="crRelationSrc__action cancel" @click="cancel()">{{ $tc('popupContent.cancel') }}</button>
                <button class="crRelationSrc__action" @click="accept()">{{ $tc('popupContent.accept') }}</button>
            </div>
        </div>
    </div>
</template>
        ...
        if (this.crRelationDest.length) {
            ...
            if (err) {
                ...
                if (data.result === 'success') {
                    const props = {
                        title: this.$tc('popupTitles.entryRelations'),
                        subtitle: this.$tc('popupContent.entryRelationsRedirecting'),
                    };
            ...
            NotificationService.show(
                ...
                `${this.$tc('popupContent.entryRelationsNoDest')}`,
                10000,
                null,
                9999,
                'error'
