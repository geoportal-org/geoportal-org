<template>
	<div class="what3words">
		<input 
			...
			@input="updateOptions()" 
			:placeholder="$t('placeholders.w3w')" 
			@blur="closeContainer()" />
		<div class="what3words__options" v-show="showOptions">
			<div class="what3words__option" @mousedown="selectOption(option)" v-for="option of options" :key="option.words">{{option.words}}</div>
		</div>
	</div>
</template>
...
import { getWhat3wordsLayer } from '@/data/map';
...
export default class What3WordsSelectComponent extends Vue {
	...
	public selectOption(option: Option) {
		...
		this.$store.dispatch(MapActions.addLayer, {
			layer: getWhat3wordsLayer(coordinates),
			id: 'what3wordsPin',
	...
	private parseSearchPhrase() {
		const parsed = this.searchPhrase
						.replace(/\s/g, '.')
						.match(/^[a-z][a-z\.]*/gi)[0];
