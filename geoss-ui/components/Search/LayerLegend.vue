<template>
    <div class="layer-legend" :class="{ collapsed: collapsed }"
        v-if="activeLayerLegend && supportedLegendTypes.includes(activeLayerLegend.legend.type)">
        <button class="layer-legend__hide-btn" @click="toggleCollapsed()"></button>
        <div>
            <div class="layer-legend__title">{{ $tc('map.legend') }}:</div>
            <div class="layer-legend__content">
                ...
                <div v-else-if="activeLayerLegend.legend.type == supportedLegendTypes[1]">
                    <div class="layer-legend__color-scale single-color">
                        <div class='value'>{{ activeLayerLegend.legend.data.value }} {{ activeLayerLegend.legend.data.unit
                        }}
                        </div>
                    </div>
                </div>
                <div v-else-if="activeLayerLegend.legend.type == supportedLegendTypes[2]"
                    class="layer-legend__two-value-scale">
                    <div class="layer-legend__color-scale">
                        <div class='max'>{{ activeLayerLegend.legend.data.max }} {{ activeLayerLegend.legend.data.unit }}
                        </div>
                        <div class='min'>{{ activeLayerLegend.legend.data.min }} {{ activeLayerLegend.legend.data.unit }}
                        </div>
                    </div>
                </div>
            </div>
