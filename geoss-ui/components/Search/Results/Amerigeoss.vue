		...
		<div class="amerigeoss-results" :class="{'with-placeholder': amerigeossResultsPlaceholders}">
			<div v-for="(result, index) of amerigeossResults" :key="result.id" class="amerigeoss-result">
				<div class="amerigeoss-result__wrapper"
					...
					<div class="amerigeoss-result__image" :class="{'amerigeoss-result__image--default': (getImage(result.logo) !== result.logo)}">
					...
					<div class="amerigeoss-result__text-data" @click="showResultDetails(result.id)">
						...
						<div v-if="result.contributor && result.contributor.orgName" class="amerigeoss-result__contributor" v-line-clamp:20="1">({{$t('dabResult.organisation')}}: {{result.contributor.orgName}})</div>
						<div v-if="result.summary && typeof result.summary === 'string'" v-line-clamp:20="3" class="amerigeoss-result__summary" :class="{'checkbox-active': checkboxActive}" v-html-to-text="result.summary"></div>
					</div>
			...
			<div v-if="amerigeossResultsPlaceholders" class="amerigeoss-result placeholder">
				<img :src="`${staticPath()}/svg/data-gray.svg`" alt="AmeriGEO" />
			</div>
		</div>
	</div>
</template>
