<template>
    <div class="custom-download">
        ...
        <div class="custom-download__params">
            ...
            <div class="custom-download__param">
                <label :title="$tc('customDownloadOptionsPopup.scaleFactor')" class="custom-download__param-title">{{
                    $tc('customDownloadOptionsPopup.scaleFactor') }}:</label>
                <div class="custom-download__param-sub">
                    <input class="custom-download__param-input" type="range" min="0.001" max="1" step="0.001"
                        v-model="scaleFactor" />
                    <input class="custom-download__param-input" type="number" min="0.001" max="1" step="0.001"
                        v-model="scaleFactor" />
                </div>
            </div>
            <div class="custom-download__param">
                <label :title="$tc('customDownloadOptionsPopup.rangeSubset')" class="custom-download__param-title">{{
                    $tc('customDownloadOptionsPopup.rangeSubset') }}:</label>
                <input class="custom-download__param-input" type="text" v-model="rangeSubset" readonly />
                <!-- Only RED_BAND actually works -->
                <CustomSelect class="custom-download__param-select" v-model="rangeSubset" :clearable="true"
                    :options="options.rangeSubset" :placeholder="$tc('customDownloadOptionsPopup.none')" />
            </div>
            <div class="custom-download__param">
                <label :title="$tc('customDownloadOptionsPopup.crs')" class="custom-download__param-title">{{
                    $tc('customDownloadOptionsPopup.crs') }}:</label>
                <CustomSelect class="custom-download__param-select" v-model="outputCRS" :clearable="false"
                    :options="options.outputCRS" :placeholder="$tc('customDownloadOptionsPopup.default')" />
            </div>
            <div class="custom-download__param">
                <label :title="$tc('customDownloadOptionsPopup.coordinatesLowerCorner')"
                    class="custom-download__param-title">{{ $tc('customDownloadOptionsPopup.coordinatesLowerCorner')
                    }}:</label>
                <div class="custom-download__param-sub coordinates">
                    <input class="custom-download__param-input" ref="lc0" type="number" :min="lowerCornerDefaults[0]"
                        :max="upperCornerDefaults[0]" step="0.0000000001" v-model="lowerCorner[0]" />
                    <input class="custom-download__param-input" ref="lc1" type="number" :min="lowerCornerDefaults[1]"
                        :max="upperCornerDefaults[1]" step="0.0000000001" v-model="lowerCorner[1]" />
                </div>
            </div>
            <div class="custom-download__param">
                <label :title="$tc('customDownloadOptionsPopup.coordinatesUpperCorner')"
                    class="custom-download__param-title">{{ $tc('customDownloadOptionsPopup.coordinatesUpperCorner')
                    }}:</label>
                <div class="custom-download__param-sub coordinates">
                    <input class="custom-download__param-input" ref="uc0" type="number" :max="upperCornerDefaults[0]"
                        :min="lowerCornerDefaults[0]" step="0.0000000001" v-model="upperCorner[0]" />
                    <input class="custom-download__param-input" ref="uc1" type="number" :max="upperCornerDefaults[1]"
                        :min="lowerCornerDefaults[1]" step="0.0000000001" v-model="upperCorner[1]" />
                </div>
            </div>
        </div>
        <div class="text-center margin-top-30">
            <button v-if="!isBulkDownloadEnabled" class="green-btn-default" @click="download()">{{
...
export default class CustomDownloadWCSComponent extends Vue {
    ...
    public rangeSubset: string = null;
    public outputCRS: string = null;
