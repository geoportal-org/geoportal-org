<template>
	<div class="custom-download">
		<div class="custom-download__title">{{$t('customDownloadOptionsPopup.title')}}:</div>
		<div class="custom-download__params">
			<div class="custom-download__param">
				<label :title="$t('customDownloadOptionsPopup.outputDownloadFormat')" class="custom-download__param-title">{{$t('customDownloadOptionsPopup.format')}}:</label>
				<CustomSelect
					class="custom-download__param-select"
					v-model="format"
					:clearable="false"
					:options="options.formatOptions" 
					:placeholder="$t('customDownloadOptionsPopup.format')" />
			</div>
			<div class="custom-download__param">
				<label :title="$t('customDownloadOptionsPopup.imageResolutionInPixels')" class="custom-download__param-title">{{$t('customDownloadOptionsPopup.outputSize')}} (height,width):</label>
			...
			<div class="custom-download__param">
				<label :title="$t('customDownloadOptionsPopup.coordinateReferenceSystemOfSubset')" class="custom-download__param-title">{{$t('customDownloadOptionsPopup.subsetCRS')}}:</label>
				<CustomSelect
					class="custom-download__param-select" 
					v-model="subsetCRS"
					...
					:placeholder="$t('customDownloadOptionsPopup.subsetCRS')" />
			</div>
			<div class="custom-download__param" v-show="subsetCRS">
				<label :title="$t('customDownloadOptionsPopup.subsetLowerCoordinateInSpecifiedCRSFormat')" class="custom-download__param-title">{{$t('customDownloadOptionsPopup.subsetLowerCoordinate')}} ({{subsetCoordinatesTitle}}):</label>
			...
			<div class="custom-download__param" v-show="subsetCRS">
				<label :title="$t('customDownloadOptionsPopup.subsetUpperCoordinateInSpecifiedCRSFormat')" class="custom-download__param-title">{{$t('customDownloadOptionsPopup.subsetUpperCoordinate')}} ({{subsetCoordinatesTitle}}):</label>
		...
		<div class="text-center margin-top-30">
			<button v-if="!isBulkDownloadEnabled" class="green-btn-default" :disabled="!outputSizeValid || !subsetCoordinatesValid" @click="download()">{{$t('customDownloadOptionsPopup.download')}}</button>
			<button v-if="isBulkDownloadEnabled && !bookmarksMode" class="green-btn-default custom-download__add-to-bulk-download" @click="addToBulkDownload()"
				...
				<i class="plus-icon"></i>
				{{$t('customDownloadOptionsPopup.addToDownloads')}}
			</button>
		</div>
	</div>
</template>
	...
	public onSubsetCRSChange() {
		...
		if(index !== 0) {
			...
			if(subsetCRSOption.id === 'EPSG:4326') {
				...
				this.subsetCoordinatesError = this.$t('customDownloadOptionsPopup.latLongRule');
			} else if(subsetCRSOption.id === 'EPSG:3857') {
				this.subsetCoordinatesTitle = 'x, y';
				this.subsetCoordinatesError = this.$t('customDownloadOptionsPopup.xYRule');
			}
