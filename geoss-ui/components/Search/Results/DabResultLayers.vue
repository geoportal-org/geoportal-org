		...
		<div v-for="(layer, index) in data" :key="index" class="layer">
			<div class="layer__wrapper">
				...
				<div class="layer__checkbox checkbox" :class="{'loading-layer': loadingLayer == layer.url}">
					<input type="checkbox" :id="`dab-result-layer-${index}`" :checked="isLayerDisplayed(layer.url) || loadingLayer == layer.url" @change="toggleSingleLayer(layer)" />
					<label :for="`dab-result-layer-${index}`">
						<i class="icomoon-eye"></i>
						<span v-if="loadingLayer == layer.url" class="layer-loading">{{$t('popupContent.loadingLayer')}}<span>.</span><span>.</span><span>.</span></span>
						<span v-else>{{$t('popupContent.loadOnMap')}}</span>
					</label>
				</div>
			</div>
		</div>
...
export default class DabResultLayersComponent extends Vue {
	...
	public async toggleSingleLayer(layer) {
