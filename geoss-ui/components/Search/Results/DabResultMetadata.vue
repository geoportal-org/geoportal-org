<template>
	<div class="metadata">
		...
		<div class="metadata__info-table" v-if="platform !== 'ZENODO' && platform !== 'WIKIPEDIA'">
			<div v-if="isSatellite">
				<div><b>{{$t('popupContent.generalInfo')}}</b></div>
				<div><b>{{$t('popupContent.platform')}}:</b>{{platform}}</div>
				<div><b>{{$t('popupContent.platformId')}}:</b>{{platformId}}</div>
				<div><b>{{$t('popupContent.productType')}}:</b>{{productType}}</div>
				<div><b>{{$t('popupContent.instrument')}}:</b>{{instrument}}</div>
				<div><b>{{$t('popupContent.cloudCoverage')}}:</b>{{cloudCoverage}}</div>
				<div><b>{{$t('popupContent.daytimeStart')}}:</b>{{daytimeStart}}</div>
				<div><b>{{$t('popupContent.daytimeStop')}}:</b>{{daytimeStop}}</div>
			</div>
			<div v-if="isSatellite">
				<div><b>{{$t('popupContent.sceneParameters')}}</b></div>
				<div><b>{{$t('popupContent.size')}}:</b>{{size}}</div>
				<div><b>{{$t('popupContent.relativeOrbit')}}:</b>{{relativeOrbit}}</div>
				<div><b>{{$t('popupContent.instrument')}}:</b>{{instrumentShort}}</div>
				<div><b>{{$t('popupContent.sensorPolarisation')}}:</b>{{sensorPolarisation}}</div>
				<div><b>{{$t('popupContent.aquisitionType')}}: </b>{{aquisitionType}}</div>
				<div><b>{{$t('popupContent.missionDataTaken')}}:</b>{{missionDataTaken}}</div>
				<div><b>{{$t('popupContent.startOrbitNumber')}}:</b>{{startOrbitNumber}}</div>
				<div><b>{{$t('popupContent.stopOrbitNumber')}}:</b>{{stopOrbitNumber}}</div>
				<div><b>{{$t('popupContent.orbitDirection')}}:</b>{{orbitDirection}}</div>
				<div><b>{{$t('popupContent.productClass')}}:</b>{{productClass}}</div>
				<div><b>{{$t('popupContent.productConsolidation')}}:</b>{{productConsolidation}}</div>
				<div><b>{{$t('popupContent.stopRelativeOrbitNumber')}}:</b>{{stopRelativeOrbitNumber}}</div>
				<div><b>{{$t('popupContent.sliceNumber')}}:</b>{{sliceNumber}}</div>
				<div><b>{{$t('popupContent.status')}}:</b>{{status}}</div>
			</div>
			<div v-if="!isSatellite">
				<div><b>{{$t('popupContent.contactInformation')}}</b></div>
				<div><b>{{$t('popupContent.contrubutor')}}:</b>{{contributor}}</div>
				<div><b>{{$t('popupContent.deliveryPoint')}}:</b>{{deliveryPoint}}</div>
				<div><b>{{$t('popupContent.city')}}:</b>{{city}}</div>
				<div><b>{{$t('popupContent.postalCode')}}:</b>{{postalCode}}</div>
				<div><b>{{$t('popupContent.country')}}:</b>{{country}}</div>
				<div><b>{{$t('popupContent.emailAddress')}}:</b>{{email}}</div>
				<div v-if="platform === 'ZENODO'"><b>{{$t('popupContent.creators')}}:</b>{{organisationName}}</div>
				<div v-else><b>{{$t('popupContent.organizationName')}}:</b>{{organisationName}}</div>
				<div><b>{{$t('popupContent.organizationUrl')}}:</b>{{organisationUrl}}</div>
				<div><b>{{$t('popupContent.role')}}:</b>{{role}}</div>
			</div>
			<div v-if="!isSatellite">
				<div><b>{{$t('popupContent.dataIdentification')}}</b></div>
				<div><b>{{$t('popupContent.fileIdentifier')}}:</b>{{fileIdentifier}}</div>
				<div><b>{{$t('popupContent.parentIdentifier')}}:</b>{{parentIdentifier}}</div>
				<div><b>{{$t('popupContent.hierarchyLevel')}}:</b>{{hierarchyLevel}}</div>
				<div><b>{{$t('popupContent.dateStamp')}}:</b>{{dateStamp}}</div>
				<div><b>{{$t('popupContent.language')}}: </b>{{language}}</div>
			</div>
		</div>
		<div class="metadata__keywords">
			<div class="metadata__keywords-title">{{$t('popupContent.descriptiveKeywords')}}</div>
		...
		<div class="metadata__additional-info" v-if="platform == 'ZENODO'">
			<div class="metadata__additional-info-title">{{$t('popupContent.additionalInfo')}}</div>
			<div class="metadata__additional-info-item"><b>{{$t('popupContent.publicationDate')}}: </b>{{publicationDate}}</div>
			<div class="metadata__additional-info-item"><b>{{$t('popupContent.doi')}}: </b><span v-html="zenodoDOI"></span></div>
			<div class="metadata__additional-info-item"><b>{{$t('popupContent.licenseForFiles')}}: </b>{{license}}</div>
			<div class="metadata__additional-info-item"><b>{{$t('popupContent.publishedIn')}}: </b>{{publishedIn}}</div>
		</div>
		<div class="metadata__coordinates">
			<div class="metadata__coordinates__title">{{$t('popupContent.mapDetails')}}</div>
			<div>
				<b>{{$t('popupContent.boundingRectangle')}}:</b> {{boundingBox}}
			</div>
			<div>
				<b>{{$t('popupContent.temporalExtent')}}:</b> {{temporalExtent}}
			</div>
		</div>
		...
		<div class="metadata__references" v-if="platform == 'ZENODO' && references.length">
			<div class="metadata__references-title">{{$t('popupContent.references')}}</div>
		...
		<div class="metadata__links" v-if="platform !== 'ZENODO'">
			<div class="metadata__links-title">{{$t('popupContent.rawOnlineResources')}}</div>
			...
			<div class="metadata__links-section" v-for="(value, key) in links" :key="key">
				<div class="metadata__links-section-title" v-if="value.items.length">
					<img v-if="value.items.length" :src="`${staticPath()}/img/m-link.png`" :alt="$t('popupContent.onlineResources')" />
...
export default class DabResultMetadataComponent extends Vue {
	...
	private async mounted() {
		...
		this.map = new ol.Map({
			target: 'infoMap',
			layers: [
				LayerTilesService.WorldStreetBasemap.getLayerTile()[0]
			],
			view: new ol.View({
