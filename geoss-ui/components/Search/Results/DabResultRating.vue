<template>
	<div class="dab-result-rating">
		<div class="dab-result-rating__comment">
			<label>{{$t('popupContent.comment')}}:</label>
		...
		<div class="d-flex flex--justify-between flex--align-end">
			<div class="dab-result-rating__rating">
				<label>{{$t('popupContent.rating')}}:</label>
			...
			<div>
				<button class="blue-btn-default" :disabled="!score" @click="rate()">{{$t('popupContent.send')}}</button>
			</div>
		</div>
...
export default class DabResultRatingComponent extends Vue {
	...
	@Prop({ default: () => [], type: Array}) private comments!: Array<{score: number, comment: string}>;
