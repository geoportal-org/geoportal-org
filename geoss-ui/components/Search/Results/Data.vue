		...
		<div class="data-results" :class="{'with-placeholder': dataResultsPlaceholders}">
			<div v-for="(result, index) of dataResults" :key="result.id" class="data-result">
				<div class="data-result__wrapper"
					...
					<div class="data-result__image" :class="{'data-result__image--default': (getImage(result.logo) !== result.logo)}">
					...
					<div class="data-result__text-data" @click="showResultDetails(result.id)">
						...
						<div v-if="result.contributor && result.contributor.orgName" class="data-result__contributor" v-line-clamp:20="1">({{$t('dabResult.organisation')}}: {{result.contributor.orgName}})</div>
						<div v-if="result.summary && typeof result.summary === 'string'" v-line-clamp:20="3" class="data-result__summary" :class="{'checkbox-active': checkboxActive}" v-html-to-text="result.summary"></div>
					</div>
			...
			<div v-if="dataResultsPlaceholders" class="data-result placeholder">
				<img :src="`${staticPath()}/svg/data-gray.svg`" alt="GEOSS" />
			</div>
		</div>
	</div>
</template>
