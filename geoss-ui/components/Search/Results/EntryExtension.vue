<template>
    <div class="entry-extension">
        <section class="pull-right" v-if="rootDataOrigin === 'geoss_cr'">
            <a class="btn" :href="advancedViewLink()" :title="$tc('popupContent.advancedView')">
                {{ $tc('popupContent.advancedView') }}
            </a>
        </section>
        <form @submit.prevent="passToModerator()">
            <section>
                <label for="keywords">
                    {{ $tc('popupContent.keywords') }}:
                </label>
                <!-- <vue-tags-input name="keywords" v-model="tag" :tags="tags" @tags-changed="newTags => tags = newTags"
                    :placeholder="''" /> -->
            </section>
            <section>
                <label for="definition">
                    {{ $tc('popupContent.definition') }}:
                </label>
                <textarea id="definition" name="definition" v-model="formData.entryExtensionForm.summary" />
            </section>
            <section>
                <label for="link">
                    {{ $tc('popupContent.link') }}:
                </label>
                <div v-for="(link, index) of formData.transferOptionsExtensionForms" :key="index" class="link-wrapper">
                    <div class="link-input" :class="{ 'with-protocol-type': showProtocolType }">
                        ...
                        <div v-if="showProtocolType">
                            <span>
                                ...
                                {{ $tc(`popupContent.${link.endpointForm.urlType}`) }}
                            </span>
                            <ul>
                    ...
                    <a class="link-add" :title="$tc('popupContent.addLinkOption')" @click="addLinkOption()">
                        <i class="icomoon-editor--add"></i>
                    </a>
                    <a class="link-remove" :title="$tc('popupContent.removeLinkOption')" @click="removeLinkOption(link)">
            ...
            <section>
                <label for="editorial-notes">
                    {{ $tc('popupContent.editorialNotes') }}:
                </label>
            ...
            <section class="pull-right">
                <button class="cta" :disabled="!formEnabled()" :class="{ disabled: !formEnabled() }">
                    {{ $tc('popupContent.passToModerator') }}
                </button>
            </section>
        </form>
    </div>
...
export default class EntryExtensionComponent extends Vue {
    ...
    @Prop({ default: null, type: String }) public rootDataOrigin!: string;

    public showProtocolType = false;
    private tag = '';
    private tags = [];
    ...
    public removeLinkOption(link: any) {
    ...
    private onTagsChange() {
        this.formData.entryExtensionForm.keywords = this.tags.map((e: any) => e.text).join();
    }
}
</script>
