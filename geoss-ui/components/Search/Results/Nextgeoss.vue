		...
		<div class="nextgeoss-results" :class="{'with-placeholder': nextgeossResultsPlaceholders}">
			<div v-for="(result, index) of nextgeossResults" :key="result.id" class="nextgeoss-result">
				<div class="nextgeoss-result__wrapper"
					...
					<div class="nextgeoss-result__image" :class="{'nextgeoss-result__image--default': (getImage(result.logo) !== result.logo)}">
					...
					<div class="nextgeoss-result__text-data" @click="showResultDetails(result.id)">
						<div v-if="result.title" v-line-clamp:20="2" class="nextgeoss-result__title">{{result.title}}</div>
						<div v-if="result.contributor && result.contributor.orgName" class="nextgeoss-result__contributor" v-line-clamp:20="1">({{$t('dabResult.organisation')}}: {{result.contributor.orgName}})</div>
						<div v-if="result.summary && typeof result.summary === 'string'" v-line-clamp:20="3" class="nextgeoss-result__summary" :class="{'checkbox-active': checkboxActive}" v-html-to-text="result.summary"></div>
					</div>
			...
			<div v-if="nextgeossResultsPlaceholders" class="nextgeoss-result placeholder">
				<img :src="`${staticPath()}/svg/data-gray.svg`" alt="NextGEOSS" />
			</div>
		</div>
	</div>
</template>
