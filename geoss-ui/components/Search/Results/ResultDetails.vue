<template>
	<div class="dab-result-details__outer-wrapper" :class="{'is-parent-ref': isParentRef && separated, 'extended-view-mode': extendedViewMode}">
		...
		<CollapseTransition>
			<div class="dab-result-details" :data-tutorial-tag="resultIdDetails === result.id ? 'result-details' : ''" :class="{'is-parent-ref': isParentRef && separated, odd: (index%2 && typeof index !== 'undefined')}" :data-id="result.id"
				v-show="(resultIdDetails === result.id && result.userContributions) || extendedViewMode">
				<div class="dab-result-details__wrapper">
					<div>
						<div v-if="isParentRef && separated" class="dab-result-details__image" :class="{'dab-result-details__image--default': (getImage(result.logo) !== result.logo)}">
							<img :src="getImage(result.logo)" @error="imageLoadError(result.logo)" :alt="title" v-image-preview />
						</div>
						<div class="dab-result-details__text-actions" :class="{'set-parent-ref-available': addParentRefAvailable(result)}">
							<div class="d-flex flex--column flex--1">
									...
									<div class="d-flex flex--wrap flex--justify-between flex--align-start flex--no-shrink">
										<div class="dab-result-details__text">
											...
											<div v-if="contributors" class="dab-result-details__contributor">
												(<span v-if="dataSource !== 'zenodo'">{{$t('dabResult.organisation')}}</span><span v-else>{{$t('dabResult.creators')}}</span>: {{ contributors }})
											</div>
										</div>
										<ViewsAndRatings :result="result"/>
									</div>
									<div v-if="summary && typeof summary === 'string'" class="dab-result-details__summary">
										<div v-if="isParentRef && separated" v-line-clamp:20="2" v-html-to-text="summary"></div>
										<div v-else v-html-to-text="summary"></div>
									</div>
									<div>
										<button @click="showDetails()" class="dab-result-details__more" :data-tutorial-tag="resultIdDetails === result.id ? 'result-see-more' : ''">
											<span>{{$t('dabResult.seeMore')}}</span>
								...
								<div class="dab-result-details__actions">
									<div class="dab-result-details__actions--main">
										<button :title=" $t('dabResult.exploreExtendedView')" @click="toggleExtendedView()"
										...
										<button :title="$t('dabResult.showOnMap')" :disabled="!layerData" @click="showOnMap()" :data-tutorial-tag="resultIdDetails === result.id ? 'result-show-on-map' : ''">
											<i class="icomoon-show-on-map"></i>
										</button>
										<button :title="$t('dabResult.addAsBookmark')" v-if="!isWidget" v-show="isSignedIn && !resultBookmarked" :disabled="!isSignedIn" @click="addBookmark()" :data-tutorial-tag="resultIdDetails === result.id ? 'result-bookmark' : ''">
											<i class="icomoon-plus"></i>
										</button>
										<button :title="$t('dabResult.removeFromBookmarks')" v-if="!isWidget" v-show="isSignedIn && resultBookmarked" :disabled="!isSignedIn" @click="removeBookmark()" :data-tutorial-tag="resultIdDetails === result.id ? 'result-bookmark' : ''">
											<i class="icomoon-minus"></i>
										</button>
									</div>
									<div class="dab-result-details__actions--side">
										<Share :data-tutorial-tag="resultIdDetails === result.id ? 'result-share' : ''" :url="shareUrl" />
										<button :title="$t('dabResult.layers')" @click="layerButtonAction()"
										...
										<button :title="$t('dabResult.downloads')"
										...
										<button class="dab-result-details__actions-workflow" :title="buttonWorkflowTitle" @click="openWorkflow(true)" v-show="workflow" :data-tutorial-tag="resultIdDetails === result.id ? 'result-workflow' : ''"></button>
										<template v-for="hub of ['data', 'information', 'services']">
											<button 
													:key="`${hub}_1`"
													v-show="!hiddenDataSources.includes(hub) && dataSourceGroup !== hub" 
													:title="$t(`dabResult.${hub}`)" 
													@click="switchToDataSource(hub, result)" 
											...
											<div v-show="showDownloads" class="dab-result-details__downloads-wrapper">
												<div>
													<span v-for="(download, index) of downloads" :key="index">
														<button v-if="download.links" @click="openDownloadLinksPopup(download.links)" :title="getDownloadButtonLabel(download.type)">
															<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`dab-result-details__file-icon icomoon-doc-${download.type}`"></i>
															<i v-else class="dab-result-details__file-icon icomoon-doc-file"></i>
														</button>
														<button v-else-if="download.url.indexOf('/dhus/odata/') !== -1" @click="openSentinelLoginPopup(download.url)" :title="getDownloadButtonLabel(download.type)">
															<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`dab-result-details__file-icon icomoon-doc-${download.type}`"></i>
															<i v-else class="dab-result-details__file-icon icomoon-doc-file"></i>
														</button>
														<button v-else-if="download.url.indexOf('/sdg/Series/DataCSV') !== -1" @click="getUnepFile(download.postData)" :title="getDownloadButtonLabel(download.type)">
															<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`dab-result-details__file-icon icomoon-doc-${download.type}`"></i>
															<i v-else class="dab-result-details__file-icon icomoon-doc-file"></i>
														</button>
														<button v-else-if="download.type === 'custom-download'" @click="initCustomDownloadPopup(download.url)" :title="$t('general.customDownload')">
															<i class="dab-result-details__file-icon icomoon-custom-download"></i>
														</button>
														<a v-else-if="download.type === 'html'" @click="downloadLinkClicked(download.url)" target="_blank" :title="getDownloadButtonLabel(download.type)">
															<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`dab-result-details__file-icon icomoon-doc-${download.type}`"></i>
															<i v-else class="dab-result-details__file-icon icomoon-doc-file"></i>
														</a>
														<a v-else-if="!isBulkDownloadEnabled" @click="downloadLinkClicked(download.url)" target="_blank" :title="getDownloadButtonLabel(download.type)">
															<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`dab-result-details__file-icon icomoon-doc-${download.type}`"></i>
															<i v-else class="dab-result-details__file-icon icomoon-doc-file"></i>
														</a>
														<a v-else-if="isBulkDownloadEnabled" target="_blank" :title="getDownloadButtonLabel(download.type)" class="expandable" :class="{'expanded': index === expandedDownloadIndex}">
															<i v-if="FileFormatsIcons.indexOf(download.type) !== -1" :class="`dab-result-details__file-icon icomoon-doc-${download.type}`"
																	@click="setExpandedDownloadIndex(index)"></i>
															<i v-else class="dab-result-details__file-icon icomoon-doc-file"
																	@click="setExpandedDownloadIndex(index)"></i>
															<p @click="downloadLinkClicked(download.url)" class="dab-result-details__direct-download-button" :title="$t('dabResult.downloadNow')">
																<span class="icomoon-arrow arrow-circled"></span> Download
															</p>
															<p class="dab-result-details__bulk-download-button" :title="$t('dabResult.downloadLater')"
																	:class="{disabled: !isSignedIn}" @click="addToDownloadsList(download.url, download.type)">
							...
							<button
								:data-tutorial-tag="resultIdDetails === result.id ? 'result-drill-down' : ''"
								:title="$t('dabResult.showInsideFolder')"
								class="dab-result-details__drill down"
...
import ExtendedViewComponent from '@/components/ExtendedView/ExtendedView.vue';
import { ExtendedViewGetters } from '@/stores/extended-view/extended-view-getters';
import { ExtendedViewActions } from '@/stores/extended-view/extended-view-actions';
import { BulkDownloadActions } from '@/stores/bulk-download/bulk-download-actions';
...
export default class SearchResultDabDetailsComponent extends Vue {
	...
	get buttonWorkflowTitle() {
		return this.isWidget ? this.$t('dabResult.workflowRedirect') : this.$t('dabResult.workflow');
	}
				...
				if(data) {
					...
					this.metadata = {
						title: `<div class="d-flex flex--justify-between flex--align-center padding-right-30">
									...
									<a class="link-white" target="_blank" href="${SearchEngineService.getMetaDataUrl(this.result.id)}">${this.$t('popupTitles.rawMetadata')}</a>
								</div>`,
						data
					};
				}
	...
	public async addBookmark() {
		...
		if(result && !err) {
			...
			const props = {
				title: this.$t('popupTitles.bookmarkResult'),
				subtitle: this.$t('popupContent.bookmarkSavedSuccess')
			};
			...
			const props = {
				title: this.$t('popupTitles.bookmarkResult'),
				subtitle: this.$t('popupContent.bookmarkSavedFail')
			};
	...
	public async removeBookmark() {
		...
		if(result) {
			...
			const props = {
				title: this.$t('popupTitles.bookmarkResult'),
				subtitle: this.$t('popupContent.bookmarkRemovedSuccess')
			};
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'general', component: GeneralPopup, props});
			let organisation = null;
			if (this.result.contributor && this.result.contributor.orgName) {
			...
			const props = {
				title: this.$t('popupTitles.bookmarkResult'),
				subtitle: this.$t('popupContent.bookmarkRemovedFail')
			};
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'error', component: ErrorPopup, props});
		}
	}
		...
		if(this.workflow) {
			if(!this.workflow.data) {
				...
				if(!err) {
					...
					const message = UtilsService.getPropByString(err, 'response.data.message') || this.$t('popupContent.serverResponseTimeout');
					const props = {
						title: this.$t('general.error'),
			...
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'workflow', title: this.$t('popupTitles.workflowandruns'), noCloseOutside: true, component: ServiceWorkflow, props});
		}
	}
	...
	private toggleLayersPopup() {
		...
		this.$store.dispatch(PopupActions.openPopup, {contentId: 'layers', title: this.$t('popupTitles.layers'), component: DabResultLayers, props});
	}
	...
	private async getStatistics() {
		...
		this.$store.dispatch(PopupActions.openPopup, {contentId: 'unsd', title: this.$t('popupTitles.unsd'), component: UnitedNationsStatistics, props});
	}
	...
	private getDownloadButtonLabel(downloadType) {
		...
		if (label === 'file') {
			label = this.$t('dabResult.file');
		} else if (label === 'other') {
			label = this.$t('dabResult.other');
		}
