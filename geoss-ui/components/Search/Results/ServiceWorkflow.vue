<template>
	<div class="service-workflow">
		...
		<div class="service-workflow__switcher">
			<button :class="{active: showDetails}" @click="toggleShowDetails(true)" :title="$t('popupContent.seeThisWorkflow')">
				<img :src="`${staticPath()}/svg/see-doc.svg`" :alt="$t('popupContent.seeThisWorkflow')" />
				<span>{{$t('popupContent.seeThisWorkflow')}}</span>
			</button>
			<button :class="{active: !showDetails}" @click="toggleShowDetails(false)" :title="$t('popupContent.runs')">
				<img :src="`${staticPath()}/svg/run-doc.svg`" :alt="$t('popupContent.runs')" />
				<span>{{$t('popupContent.runs')}}</span>
			</button>
		</div>
		...
		<div class="service-workflow__details" v-show="showDetails">
			<div class="margin-bottom-30">
				...
				<div class="bpmn-legend d-flex flex--justify-center flex--align-center flex--wrap">
					<span class="required">{{ $t('popupContent.requiredFields') }}</span>
					<span class="optional">{{ $t('popupContent.optionalFields') }}</span>
					<span class="expert">{{ $t('popupContent.expertOptions') }}</span>
					<span class="output">{{ $t('popupContent.outputs') }}</span>
				</div>
			</div>

			<div class="service-workflow__run-title">
				<div class="service-workflow__title">
					<span>{{ $t('popupContent.workflowInput') }}</span>
				</div>
			</div>

			<div class="service-workflow__resource" v-if="workflowInputs.length">
				<div class="service-workflow__resource-header">
					<div>{{$t('popupContent.inputName')}}</div>
					<div>{{$t('popupContent.chosenResources')}}</div>
					<div>{{$t('popupContent.actions')}}</div>
				</div>
				<div class="service-workflow__resource-data" v-for="input of workflowInputs" :key="input.id" v-show="!isExpert(input.id) || (isExpert(input.id) && showExpertOptions)">
					<div class="d-flex flex--align-center">{{input.name}} <span v-if="isRequired(input.id)">*</span></div>
					<div>
						<div v-if="workflowResource && workflowResource[input.id] && workflowResource[input.id].length && !workflowResource[input.id][0].dataInput">
							<div class="d-flex flex--align-center" v-for="resource of workflowResource[input.id]" :key="resource.id">
								<button @click="removeWorkflowResource(resource, input.id)" :title="$t('popupContent.removeResource')">
									<i class="cross"></i>
								</button>
						...
						<div v-else-if="isRequired(input.id)">
							<span class="field-required">
								{{ $t('popupContent.thisFieldIsRequired') }}
							</span>
						</div>
						<div v-else>
							<div v-if="getDefaultValue(input.id)">
								<div v-if="activeEdit(input.id)" class="d-flex flex--align-center">
									<input v-if="getValueSchema(input.id) === 'bbox'" :placeholder="$t('popupContent.coordinatesWSEN')" :id="input.id" @change="updateResourceValue(input.id, $event)"/>
									<input v-else :placeholder="getDefaultValue(input.id, true)" :id="input.id" @change="updateResourceValue(input.id, $event)" />
								...
								<div v-else @click="addEditResource(input.id)" class="default-value d-flex flex--align-center">
									<i class="pencil">&#9998;</i>
									<span v-html="getDefaultValue(input.id)"></span>
								</div>
							</div>
					...
					<div class="service-workflow__resource-actions flex--justify-center">
						<button v-if="getValueSchema(input.id) === 'bbox'" class="service-workflow__select-resources" @click="setBoundingBox(input.id)">{{$t('popupContent.setBoundingBox')}}</button>
						<span v-else-if="isExpert(input.id)" class="expert-option-info">{{ $t('popupContent.additionalExpertOption') }}</span>
						<button v-else class="service-workflow__select-resources" @click="selectResources(input.id)"><span>{{$t('popupContent.selectResources')}}</span> <span class="arrow"></span></button>
					</div>
				</div>
			</div>

			<div v-if="hasRequiredFields" class="service-workflow__required-fields d-flex flex--align-center flex--wrap flex--justify-between">
				<div>
					* {{ $t('popupContent.requiredFields') }}
				</div>
				<div>
					<button v-if="!showExpertOptions && hasExpertOptions" class="service-workflow__expert-options show" @click="toggleExpertOptions()">
						{{$t('popupContent.showExpertOptions')}}
					</button>
					<button v-if="showExpertOptions && hasExpertOptions" class="service-workflow__expert-options hide" @click="toggleExpertOptions()">
						{{$t('popupContent.hideExpertOptions')}}
					</button>
				</div>
			</div>
			<div class="service-workflow__platforms">
				<div class="service-workflow__title">
					<span>{{ $t('popupContent.cloudPlatformSelection') }}</span>
					<small v-if="platformDataLoading !== 0" class="platform-loading">{{ $t('popupContent.fetchingPlatformsData') }}<span>.</span><span>.</span><span>.</span></small>
				</div>
				<div class="service-workflow__platforms-choice">
					<label
						...
						<small>{{ $t('popupContent.optimal') }}</small>
			...
			<div class="service-workflow__run-title">
				<div class="service-workflow__title">
					<span>{{ $t('popupContent.runName') }}</span>
				</div>
				<div class="service-workflow__run-title-input">
					<input type="text" id="run-title" :placeholder="$t('popupContent.runName')" v-model="workflowRunName" />
				</div>
			</div>
			<div class="d-flex flex--justify-end flex--wrap">
				<button class="service-workflow__run" :class="{disabled: !allRequiredFilled}" :disabled="!allRequiredFilled" @click="run()">{{$t('popupContent.run')}}</button>
			</div>
		</div>
		<div v-show="!showDetails">
			<div class="d-flex flex--justify-between flex--align-center service-workflow__add-run">
				...
				<button class="service-workflow__add-run-btn" :disabled="!addRunId || !addRunName" @click="addRun()">{{$t('popupContent.add')}}</button>
			</div>
			<div v-show="!savedRuns || !savedRuns.length">{{$t('popupContent.noRunsAvailable')}}</div>
			<div class="service-workflow__saved-runs" v-if="savedRuns">
				<div>
					<div class="service-workflow__saved-run" v-for="savedRun of savedRuns" :key="savedRun.id">
						...
						<div class="service-workflow__saved-run-id">{{$t('popupContent.id')}}: {{savedRun.id}}</div>
						<div class="service-workflow__saved-run-status">{{$t('popupContent.status')}}: {{savedRun.status}}<span v-if="savedRun.result"> - {{ savedRun.result }}</span></div>
						<div class="service-workflow__saved-run-button-holder d-flex">
							<button class="service-workflow__saved-run-btn" :class="{active: savedRun.showLogs}" @click="toggleLogs(savedRun)" v-if="savedRun.messageList && savedRun.messageList.length">{{$t('popupContent.messageLog')}}</button>
							<button class="service-workflow__saved-run-btn" :class="{active: savedRun.showOutputs}" @click="toggleOutputs(savedRun)" v-if="savedRun.outputs && savedRun.outputs.length && savedRun.result !== 'FAIL'">{{$t('popupContent.outputs')}}</button>
						</div>
						...
						<CollapseTransition v-if="savedRun.outputs && savedRun.status === 'COMPLETED' && savedRun.result !== 'FAIL'">
							<div v-show="savedRun.showOutputs" class="service-workflow__saved-outputs">
								<div v-for="output of savedRun.outputs" :key="output.id" class="service-workflow__saved-output">
									<div v-if="output.value && output.valueSchema === 'url'">
										<a :href="output.value" target="_blank" class="service-workflow__saved-output-link">{{output.name}}</a>
										<div class="service-workflow__saved-output-link-description">{{output.description}}</div>
									</div>
									<div v-else-if="output.value && output.value.url" class="d-flex flex--align-center">
										<button @click="showOnMap(savedRun.runId, output)" :title="$t('dabResult.showOnMap')">
											<i class="icomoon-show-on-map"></i>
										</button>
										<span>{{output.name}}</span>
									</div>
									<div v-else class="d-flex flex--align-center">
										<button disabled="true" :title="$t('dabResult.showOnMap')">
											<i class="icomoon-show-on-map"></i>
										</button>
										<span>{{output.name}}</span>
									</div>
...
import { IrisFiltersActions } from '@/stores/iris-filters/iris-filters-actions';
...
import { parseXMLToJSON, makeRequest } from '@/services/general.api.service';
...
export default class ServiceWorkflowComponent extends Vue {
		...
		if (placeholder) {
			...
			if (this.getValueSchema(id) === 'bbox') {
				if (this.workflowCoordinates && this.workflowCoordinates.W && this.workflowCoordinates.S && this.workflowCoordinates.E && this.workflowCoordinates.N) {
					...
					return this.$t('popupContent.worldwide');
				}
	...
	public async run() {
		if(!this.isSignedIn) {
			...
			const message = `${this.$t('popupContent.mustBeLoggedIn1')}<a href="${fullLoginUrl}">${this.$t('popupContent.mustBeLoggedIn2')}</a> ${this.$t('popupContent.mustBeLoggedIn3')}`;
		...
		if(!err) {
			...
			if(result) {
				NotificationService.show(
					`${this.$t('popupTitles.savedRuns')}`,
					`${this.$t('popupContent.saveRunSuccess')}`,
					10000,
					null,
					9999,
					'success'
		...
		if(!err) {
			GeossSearchApiService.addSavedRun(this.workflowRunName, this.workflow, this.addRunId).then(() => {
				...
				if(status === 'COMPLETED' || status === 'SUCCESS') {
					return GeossSearchApiService.getRunStatus(this.addRunId).then(({outputs}) => {
						run.outputs = outputs;
						run.showOutputs = false;
					});
				}
			...
			for (const input in defaultValues) {
				if (!workflowResource[input]) {
					workflowResource[input] = [{
						...
						'title': this.$t('popupContent.default'),
						'dm:workflowinput': {
...
.service-workflow {
	...
	svg[data-element-id="autogenerated-1559206811037-process"], // EODESM
	svg[data-element-id="autogenerated-1540301452629-process"] { // EO-SDM
		left: 70px;
	}

	svg[data-element-id="autogenerated-1523951879929-process"], // LandMetrics
	svg[data-element-id="autogenerated-1528478431956-process"] { // PHENOLOGYMETRICS
		left: 190px;
	}
	
	svg[data-element-id="autogenerated-1527757840088-process"] { // COINS
		left: 240px;
	}

	svg[data-element-id="autogenerated-1664543852740-process"], // Trends Earth v1
	svg[data-element-id="autogenerated-1528362962211-process"], // INSTAR
	svg[data-element-id="autogenerated-1543344870684-process"], // WIMMED
	svg[data-element-id="autogenerated-1523461201638-process"] { // HYDROMAP
		left: 270px;
	}	
}
</style>
