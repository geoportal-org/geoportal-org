		...
		<div class="services-results" :class="{'with-placeholder': servicesResultsPlaceholders}">
			<div v-for="(result, index) of serviceResults" :key="result.id" class="services-result">
				<div class="services-result__wrapper"
					...
					<div class="services-result__image" :class="{'services-result__image--default': (getImage(result.logo) !== result.logo)}">
					...
					<div class="services-result__text-data" @click="showResultDetails(result.id)">
						<div v-if="result.title" v-line-clamp:20="2" class="services-result__title">{{result.title}}</div>
						<div v-if="result.contributor && result.contributor.orgName" class="services-result__contributor" v-line-clamp:20="1">({{$t('dabResult.organisation')}}: {{result.contributor.orgName}})</div>
						<div v-if="result.summary && typeof result.summary === 'string'" v-line-clamp:20="3" class="services-result__summary" :class="{'checkbox-active': checkboxActive}" v-html-to-text="result.summary"></div>
					</div>
			...
			<div v-if="servicesResultsPlaceholders" class="services-result placeholder">
				<img :src="`${staticPath()}/svg/services-gray.svg`" alt="SERVICES" />
			</div>
		</div>
	</div>
</template>
