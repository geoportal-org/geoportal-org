<template>
	<div class="unsd">
		<div class="unsd__title">{{$t('unitedNationsStatisticsPopup.title')}}:</div>
		<div class="unsd__params">
			<div class="unsd__param">
				<label :title="$t('unitedNationsStatisticsPopup.timeSeriesTooltip')" class="unsd__param-title">{{$t('unitedNationsStatisticsPopup.timeSeries')}}:</label>
				<CustomSelect
					class="unsd__param-select"
					v-model="timeSeries"
					:clearable="false"
					:appendToBody="true"
					:options="options.timeSeriesOptions"
					:placeholder="$t('unitedNationsStatisticsPopup.timeSeriesPlaceholder')" />
			</div>
			<div class="unsd__param">
				<label :title="$t('unitedNationsStatisticsPopup.yearTooltip')" class="unsd__param-title">{{$t('unitedNationsStatisticsPopup.year')}}:</label>
				<CustomSelect
					class="unsd__param-select"
					v-model="year"
					:clearable="false"
					...
					:placeholder="$t('unitedNationsStatisticsPopup.yearPlaceholder')" />
			</div>
			<div class="unsd__param">
				<label :title="$t('unitedNationsStatisticsPopup.attributesTooltip')" class="unsd__param-title">{{$t('unitedNationsStatisticsPopup.attributes')}}:</label>
				<CustomSelect
					class="unsd__param-select"
					v-model="attributesIndex"
					:clearable="false"
					:appendToBody="true"
					:options="options.attributesOptionsStrings"
					:placeholder="$t('unitedNationsStatisticsPopup.attributesPlaceholder')" />
			</div>
		</div>
		<div class="text-center margin-top-30">
			<button class="green-btn-default" :disabled="addLayerDisabled" @click="prepareResultsAndCreateLayer">{{$t('unitedNationsStatisticsPopup.addLayer')}}</button>
		</div>
	</div>
</template>
...
export default class UnitedNationsStatisticsComponent extends Vue {
	...
	private async prepareTimeSeries() {
		this.options.timeSeriesOptions = [{text: this.$t('unitedNationsStatisticsPopup.timeSeriesPlaceholder'), id: 'default'}, ...this.timeSeriesArray];
		this.timeSeries = this.options.timeSeriesOptions[0].id;
	}

	private async loadAccessibleYears() {
		this.options.yearOptions = [{text: this.$t('unitedNationsStatisticsPopup.yearPlaceholder'), id: 'default'}];

		const [err, accesibleYears] = await to(GeossSearchApiService.getStatisticsTimePeriods([this.timeSeries], this.areaCodes));
		if (err) {
			const props = {
				title: this.$t('general.error'),
				subtitle: this.$t('unitedNationsStatisticsPopup.unavailable')
			};
			PopupCloseService.closePopup('unsd');
	...
	private async loadAccessibleAttributes() {
		...
		const dimensionsArrayStrings = [{text: this.$t('unitedNationsStatisticsPopup.attributesPlaceholder'), id: 'default'}];
		let id = 0;
