<template>
	<div class="user-contributions" v-if="userExtensions.length && showSection()">
		<div class="user-contributions__label">
			{{ $t(`popupContent.userContributedTitle${model.charAt(0).toUpperCase() + model.slice(1)}`) }}
		</div>
		<div class="user-contributions__item" v-for="(extension, index) in userExtensions" :key="index" v-show="showItem(extension)">
			<div class="user-contributions__item-metadata">
				...
				<div v-if="extension.userId.toString() === userId && showDeleteIcon && model !== 'transferOptions'" :title="$t('popupContent.userContributedRemoveEntryExtension')" class="delete-extension" @click="removeExtension(extension)">
			...
			<div v-if="model === 'transferOptions'" class="user-contributions__item-content">
				<div v-for="(link, index) in extension[model]" :key="index">
					<div class="metadata__link" :title="link.displayTitle">
						...
						<div v-if="extension.userId.toString() === userId && showDeleteIcon && model === 'transferOptions'" class="delete-extension" @click="removeExtension(extension, index)">
							<i class="icomoon-editor--trash"></i>
						</div>
					</div>
				</div>
...
export default class UserContributionsMetadata extends Vue {
	@Prop(String) private model!: string;
	@Prop(Object) private data!: any;
	private showDeleteIcon = true;
	private userExtensions = this.model === 'comment' ? this.userContributionsComments : this.userContributionsExtensions;
	...
	private async removeExtension(extension, index = 0) {
		let err;
		let data;
		...
		if(err || (data && data.result === 'error')) {
			...
			if (
				...
				const props = {
					...
					subtitle: this.$t('popupContent.userContributedRemoveUnavailable'),
					actions: [{
