		...
		<div class="wikipedia-results" :class="{'with-placeholder': wikipediaResultsPlaceholders}">
			<div v-for="(result, index) of wikipediaResults" :key="result.id" class="wikipedia-result">
				<div class="wikipedia-result__wrapper"
					...
					<div class="wikipedia-result__image" :class="{'wikipedia-result__image--default': (getImage(result.logo) !== result.logo)}">
					...
					<div class="wikipedia-result__text-data" @click="showResultDetails(result.id)">
						<div v-if="result.title" v-line-clamp:20="2" class="wikipedia-result__title">{{result.title}}</div>
						<div v-if="result.summary && typeof result.summary === 'string'" v-line-clamp:20="3" class="wikipedia-result__summary" :class="{'checkbox-active': checkboxActive}" v-html-to-text="result.summary"></div>
					</div>
			...
			<div v-if="wikipediaResultsPlaceholders" class="wikipedia-result placeholder">
				<img :src="`${staticPath()}/svg/information-gray.svg`" alt="WIKIPEDIA" />
			</div>
		</div>
	</div>
</template>
