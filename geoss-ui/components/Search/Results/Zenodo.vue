	...
	<div>
		<div class="zenodo-results" :class="{'with-placeholder': zenodoResultsPlaceholders}">
			<div v-for="(result, index) of zenodoResults" :key="result.id" class="zenodo-result">
				<div class="zenodo-result__wrapper"
					...
					<div class="zenodo-result__text-data" @click="showResultDetails(result.id)">
						<div v-if="result.metadata.title" v-line-clamp:20="2" class="zenodo-result__title">{{result.metadata.title}}</div>
						<div v-if="result.metadata && result.metadata.creators" class="zenodo-result__contributor" v-line-clamp:20="1">
							({{$t('dabResult.creators')}}: <span v-for="(creator, index) of result.metadata.creators" :key="index" >{{ creator.name }}</span>)
						</div>
						<div v-if="result.metadata.description && typeof result.metadata.description === 'string'" v-line-clamp:20="3" class="zenodo-result__summary" :class="{'checkbox-active': checkboxActive}" v-html-to-text="result.metadata.description"></div>
					</div>
			...
			<div v-if="zenodoResultsPlaceholders" class="zenodo-result placeholder">
				<img :src="`${staticPath()}/svg/information-gray.svg`" alt="ZENODO" />
			</div>
		</div>
	</div>
</template>
