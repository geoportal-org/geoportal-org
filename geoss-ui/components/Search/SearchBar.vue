<template>
	<div class="search-bar" data-tutorial-tag="searchbar">
		...
		<input data-tutorial-tag="searchbar-input" aria-label="phrase" class="search-bar__input" type="text" v-model="phrase" @keyup="onKeyup($event)" @keydown="onKeydown($event)" @focus="getSuggestions()" @blur="resetSuggestions()" :placeholder="$t('placeholders.searchBar')" />
		<button data-tutorial-tag="searchbar-search" class="search-bar__search-trigger disabled-transparent" @click="search()" :disabled="(!generalFiltersNotEmpty && this.phrase === '') || !dataSource" :title="$t('searchBar.search')">
		...
		<button data-tutorial-tag="searchbar-save" class="search-bar__save-search disabled-transparent" :disabled="!searchResultsActive || searchJustSaved" v-if="isSignedIn" @click="saveSearch()" :title="$t('searchBar.saveSearch')">
			<i class="icomoon-save"></i>
		</button>
		<button data-tutorial-tag="searchbar-clear" class="search-bar__clear-search-container disabled-transparent" :title="$t('searchBar.clearSearch')"
				:disabled="!generalFiltersNotEmpty && !searchResultsActive && this.phrase === ''" @click="resetGeneralFiltersAndResults()">
			<i class="icomoon-close"></i>
		</button>
		<div class="search-bar__autocomplete" v-show="suggestions.length || seeAlso.length">
			<div class="suggestions" v-if="suggestions.length">
				<i class="icomoon-star-empty"></i>
				<b>{{ $t('searchBar.popular') }}</b>
...
import { DataSource, DataSources } from '@/interfaces/DataSources';
...
export default class SearchBarComponent extends Vue {
	...
	public suggestions = [];
	public seeAlso = [];
	public selectedSuggestion: string = null;
	...
	public onKeyup(event: KeyboardEvent) {
		if(this.phrase && this.lastPhrase !== this.phrase && event.keyCode !== 13 && this.phrase.length > 1) {
			this.getSuggestions();
	...
	public search(event?: KeyboardEvent) {
		if((this.generalFiltersNotEmpty || this.phrase) && this.dataSource) {
			...
			if (GeossSearchApiService.isIdenticalToCacheParams(this.$store.getters[SearchGetters.filtersParams])) {
	...
	public async saveSearch() {
		let bbox = '';
		const { S, W, N, E } = this.$store.getters[GeneralFiltersGetters.state].selectedAreaCoordinates;
		if (S && W && N && E) {
			bbox = `${S.toFixed(13)}, ${W.toFixed(13)}, ${N.toFixed(13)}, ${E.toFixed(13)}`;
		}
		...
		let targetId = '';
		if (this.$store.getters[SearchGetters.parentRef] && this.$store.getters[SearchGetters.parentRef].id) {
			targetId = this.$store.getters[SearchGetters.parentRef].id;
		}

		let filterSet = '';
		if (Object.keys(this.$store.getters[FacetedFiltersGetters.saveParams]).length) {
			filterSet = 'faceted-filters';
		} else if (Object.keys(this.$store.getters[GranulaFiltersGetters.saveParams]).length) {
			filterSet = 'granula-filters';
		} else if (Object.keys(this.$store.getters[IrisFiltersGetters.saveParams]).length) {
			filterSet = 'iris-filters';
		}

		const [, result] = await to(GeossSearchApiService.addSavedSearch({
			name: this.$store.getters[GeneralFiltersGetters.state].phrase,
			popularSearchId: this.$store.getters[MyWorkspaceGetters.search].popularSearchId,
			query: this.$store.getters[GeneralFiltersGetters.state].phrase,
			fullAndOpenDataset: this.$store.getters[GeneralFiltersGetters.state].geossDataCore,
			dateFrom: (this.$store.getters[GeneralFiltersGetters.state].dateFrom ? this.$store.getters[GeneralFiltersGetters.state].dateFrom.toString() : ''),
			dateTo: (this.$store.getters[GeneralFiltersGetters.state].dateTo ? this.$store.getters[GeneralFiltersGetters.state].dateTo.toString() : ''),
			datePeriod: (this.$store.getters[GeneralFiltersGetters.state].datePeriod ? this.$store.getters[GeneralFiltersGetters.state].datePeriod.toString() : ''),
			aoiOption: this.$store.getters[GeneralFiltersGetters.state].locationType,
			aoiBoundingBox: bbox,
			aoiGeolocation: this.$store.getters[GeneralFiltersGetters.state].googlePlacesInput,
			aoiW3W: '', // Unused, but field still exists in DB
			aoiRelation: this.$store.getters[GeneralFiltersGetters.state].boundingBoxRelation,
		...
		if(result) {
			const props = {
				title: this.$t('popupTitles.saveSearch'),
				subtitle: this.$t('popupContent.saveSearchSuccess')
			};
			...
			const props = {
				title: this.$t('popupTitles.saveSearch'),
				subtitle: this.$t('popupContent.saveSearchFail')
			};
			...
			if (!this.suggestionsActiveCalls) {
				if (!this.cancelSuggestions) {
					if(suggested && Array.isArray(suggested)) {
						...
						if((suggestions.length !== 1 || suggestions[0] !== this.phrase) && this.$el.querySelector('input') === document.activeElement) {
	...
	public onKeydown(event: KeyboardEvent) {
		...
		if(event.keyCode === 38) { // up arrow
			index = (this.selectedSuggestion ? this.suggestions.indexOf(this.selectedSuggestion) : this.suggestions.length);
			index = (index === 0 ? (this.suggestions.length - 1) : (index - 1));
		} else if(event.keyCode === 40) { // down arrow
			index = (this.selectedSuggestion ? this.suggestions.indexOf(this.selectedSuggestion) : -1);
