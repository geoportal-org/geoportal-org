		...
		<div class="search-container" :class="{'active': containerActive, 'container-hidden': containerFullyHidden, 'in-transition': containerInTransition}">
			<DraggableResizable :handles="['br']" :disabled="dragResizeDisabled" :min-height="300" :min-width="500" :resizable="searchResultsActive"
				...
				<button
					...
					<span v-if="containerFullyHidden && searchResultsActive">{{ $t('general.searchResults') }}</span>
					<span v-if="containerFullyHidden && !searchResultsActive">{{ $t('searchBar.search') }}</span>
				...
				<SeeAlso v-if="this.windowWidth < 1200" />
			</DraggableResizable>
		</div>
		<SeeAlso v-if="this.windowWidth >= 1200" />
	</div>
...
import { Graticule } from 'openlayers';
