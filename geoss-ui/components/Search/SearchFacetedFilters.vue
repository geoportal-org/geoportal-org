<template>
    <div class="faceted-filters filters">
        <div class="faceted-filters__filters-hide" v-show="!anyFilterAvailable">{{ $tc('general.noFiltersAvailable') }}
        </div>
        <div class="faceted-filters__wrapper" v-show="anyFilterAvailable">
            <CustomSelect class="faceted-filters__filter" v-model="keyword" :options="keywordOptions" :filterable="true"
                :placeholder="$tc('facetedFilters.keyword')" :appendToBody="appendToBody"
                data-tutorial-tag="filters-faceted-keyword" />
            <CustomSelect class="faceted-filters__filter" v-model="format" :options="formatOptions" :filterable="true"
                :placeholder="$tc('facetedFilters.format')" :appendToBody="appendToBody"
                data-tutorial-tag="filters-faceted-format" />
            <CustomSelect class="faceted-filters__filter" v-model="source" :options="sourceOptions" :filterable="true"
                :placeholder="$tc('facetedFilters.source')" :appendToBody="appendToBody"
                data-tutorial-tag="filters-faceted-source" />
            <CustomSelect class="faceted-filters__filter" v-model="protocol" :options="protocolOptions" :filterable="true"
                :placeholder="$tc('facetedFilters.protocol')" :appendToBody="appendToBody"
                data-tutorial-tag="filters-faceted-protocol" />
            <CustomSelect class="faceted-filters__filter" v-model="organisation" :options="organisationOptions"
                :filterable="true" :placeholder="$tc('facetedFilters.organisation')" :appendToBody="appendToBody"
                data-tutorial-tag="filters-faceted-organisation" />
            <CustomSelect class="faceted-filters__filter" v-model="score" :options="scoreOptions" :filterable="true"
                :placeholder="$tc('facetedFilters.serviceHealth')" :appendToBody="appendToBody"
            ...
            if (filterOptionsRaw && filterOptionsRaw[`dm:item`]) {
                ...
                for (const item of filterOptionsRaw[`dm:item`]) {
                    ...
                    if (filter === 'organisationName') {
                        ...
                        if (name === '0 - 20') {
                            name = this.$tc('fileDownloadsPopup.veryUnreliable');
                        } else if (name === '21 - 40') {
                            name = this.$tc('fileDownloadsPopup.frequentlyUnavailable');
                        } else if (name === '41 - 60') {
                            name = this.$tc('fileDownloadsPopup.sometimesUnavailable');
                        } else if (name === '61 - 80') {
                            name = this.$tc('fileDownloadsPopup.mostlyAvailable');
                        } else if (name === '81 - 100') {
                            name = this.$tc('fileDownloadsPopup.veryReliable');
                        }
                    }
