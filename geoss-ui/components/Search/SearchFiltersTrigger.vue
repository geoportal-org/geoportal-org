<template>
	<div class="trigger" :class="{active: !onlyAdvanced && (active || !activeSection)}">
		<span data-tutorial-tag="searchbar-advanced-search" :class="{'disable-container': onlyAdvanced && generalFiltersInChange}" @click="toggleFilters()" v-show="onlyAdvanced">{{$t('generalFilters.advancedSearch')}}</span>
		<span @click="toggleFilters()" v-show="!onlyAdvanced && !active">{{$t('generalFilters.filters')}}</span>
		<button
...
export default class SearchFiltersTriggerComponent extends Vue {
	...
	@Prop({type: String}) public activeSection!: boolean;
