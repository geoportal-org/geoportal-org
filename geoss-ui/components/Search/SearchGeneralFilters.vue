	...
	<div class="filters general-filters" :class="{'only-advanced': onlyAdvanced}">
		<div class="general-filters__absolute-box" :class="{stretched: generalFiltersInChange}">
			...
			<div class="general-filters__filter--geoss-submit d-flex margin-right-15" v-if="generalFiltersInChange">
				<button @click="cancelChanges()" class="general-filters__action cancel">{{$t('generalFilters.cancel')}}</button>
				<button :disabled="!isNonEmpty" @click="acceptChanges()" class="general-filters__action accept">{{$t('generalFilters.accept')}}</button>
			</div>
		</div>
		<div class="general-filters__wrapper">
			...
			<div class="general-filters__filter general-filters__filter--coordinates location"
				...
				<div class="coordinates-buttons">
					<button @click="clearCoordinates()" :class="{'active': isCoordinateFilled()}">✕ {{$t('generalFilters.cancel')}}</button>
				</div>
			</div>
...
export default class SearchGeneralFiltersComponent extends Vue {
	...
	get availableLocationTypeOptions() {
		const locationTypeOptions = this.$store.getters[GeneralFiltersGetters.locationTypeOptions];
		const options = [];
		for (const locationType of locationTypeOptions) {
			const textPath = locationType === 'continent_country' ? 'generalFilters.continentCountry' : 'generalFilters.' + locationType;
			options.push({
				id: locationType,
				text: this.$t(textPath)
			});
		}
		return options;
	}
	...
	public clearCoordinates() {
		this.$store.dispatch(GeneralFiltersActions.setSelectedAreaCoordinates, { W: null, S: null, E: null, N: null });
	}
	...
	public async cancelChanges() {
		...
		if (this.workflow) {
			...
			this.$store.dispatch(GeneralFiltersActions.setWorkflowMapDraw, false);
	...
	private rememberState(lastState?) {
		if(!this.lastState) {
			...
			setTimeout(() => {
				if(this.lastState) {
					const currentGeneralFilters = JSON.stringify(this.$store.getters[GeneralFiltersGetters.values]);
					...
					*/
					const inChangeProcess = this.lastState.datePeriod === '' ? currentGeneralFilters !== JSON.stringify(this.lastState) : false;
					this.$store.dispatch(GeneralFiltersActions.setInChangeProcess, inChangeProcess);
				}
			}, 0);
		}
	}
	...
	private getGooglePlacesApiError() {
