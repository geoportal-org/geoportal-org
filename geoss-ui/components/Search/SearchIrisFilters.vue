<template>
	<div class="iris-filters filters">
		<div class="iris-filters__wrapper" v-show="activeSection !== 'advanced'">
			<CustomSelect
				class="iris-filters__filter" 
				v-model="magnitudeType"
				:options="magnitudeTypeOptions"
				:appendToBody="appendToBody"
				:placeholder="$t('irisFilters.magnitudeType')" 
			...
			<CustomSelect
				...
				:options="sortOptions" 
				:appendToBody="appendToBody"
				:placeholder="$t('irisFilters.sortBy')"
				data-tutorial-tag="filters-iris-sort" />
			<div class="iris-filters__filter slider" data-tutorial-tag="filters-iris-magnitude-range" >
				...
				<vue-slider ref="magnitudeSlider" :min="0" :max="10" :value="magnitude" tooltip="focus"
					...
					@drag-end="dragStop('magnitude', $event)" 
			...
			<div class="iris-filters__filter slider" data-tutorial-tag="filters-iris-depth-range" >
				...
				<vue-slider ref="depthSlider" :min="0" :max="100" :value="depth" tooltip="focus"
					@drag-start="dragStart()"
					@drag-end="dragStop('depth', $event)" 
					@change="sliderChange('depth', $event)" />
			</div>
			<DateSlider class="iris-filters__filter full-width" :min-year="minYear" :max-year="maxYear" :date-from="dateFrom" :date-to="dateTo"
				:listenDragEnd="true" @on-change-dates="changeDates($event)" data-tutorial-tag="filters-iris-date-range" />
		</div>
	</div>
</template>
