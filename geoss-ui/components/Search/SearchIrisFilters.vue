<template>
    <div class="iris-filters filters">
        <div class="iris-filters__wrapper" v-show="activeSection !== 'advanced'">
            ...
            <CustomSelect class="iris-filters__filter" v-model="sort" :options="sortOptions"
                :appendToBody="appendToBody" :placeholder="$tc('irisFilters.sortBy')"
            ...
            <div class="iris-filters__filter slider" data-tutorial-tag="filters-iris-depth-range">
                ...
                <vue-slider ref="depthSlider" :min="0" :max="100" :value="depth" tooltip="focus"
                    @drag-start="dragStart()" @drag-end="dragStop('depth', $event)"
...
export default class SearchIrisFiltersComponent extends Vue {
    ...
    private onIsAvailableChange(val: boolean, oldVal: boolean) {
        if (!oldVal && val) {
            this.$store.dispatch(
                IrisFiltersActions.setMinMagnitude,
                this.magnitude[0] || 0
            )
            this.$store.dispatch(
                IrisFiltersActions.setMaxMagnitude,
                this.magnitude[1] || 10
            )
            this.$store.dispatch(
                IrisFiltersActions.setMinDepth,
                this.depth[0] || 0
            )
            this.$store.dispatch(
                IrisFiltersActions.setMaxDepth,
                this.depth[1] || 100
            )
        }
    }
