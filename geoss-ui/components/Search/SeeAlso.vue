<template>
	<div v-show="(this.recentSeeAlsoPhrases.length || this.recentSeeAlsoRecommendations.length) && this.currentResults" class="see-also" :class="{'active': containerActive, 'container-hidden': containerFullyHidden, 'in-transition': containerInTransition}">
		<DraggableResizable
			:handles="['br']"
			:disabled="false"
			...
			<button class="see-also-toggler"
				...
				<span v-if="containerFullyHidden">{{ $t('searchBar.seeAlso') }}</span>
				<span v-if="!containerFullyHidden" class="see-also-toggler__arrow"></span>
			</button>
			<div slot="handle" class="resize-icon">
				<span></span>
			</div>
			<div class="see-also__drag-handle" :title="$t('searchBar.moveAround')"></div>
			<div class="see-also__scrollable-wrapper">
				<button class="see-also__scroll-button scroll-left" 
					...
					></button>
				<div v-if="this.recentSeeAlsoPhrases.length" class="see-also__scrollable-track">
					<b class="see-also__title" data-tutorial-tag="see-also">{{ $t('searchBar.seeAlso') }}</b>
					<div v-for="(item, index) in recentSeeAlsoPhrases" :key="index" @click="selectSuggestion(item)" class="see-also__item">
						<i>{{item}}</i>
					</div>
				</div>
				<div v-if="this.recentSeeAlsoRecommendations.length" class="see-also__scrollable-track recommendations">
					<b class="see-also__title recommendations" data-tutorial-tag="see-also-recommendations">{{ $t('searchBar.recommendationsForYou') }}</b>
...
export default class SeeAlsoComponent extends Vue {
	...
	public selectRecommendation(item) {
