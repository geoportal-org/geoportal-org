...
export default class WorkflowCheckboxComponent extends Vue {
	...
	public selectWorkflowResource(result) {
		if(this.workflowInputType === 'individual') {
			...
			if(this.isWorkflowResourceChecked(result)) {
				this.workflowResource = this.workflowResource.filter(item => item.id !== result.id);
			} else {
	...
	public isWorkflowResourceChecked(result) {
		return this.workflowResource && this.workflowResource.find(item => item.id === result.id);
	}
}
</script>
