<template>
	<div class="views-ratings">
		...
		<a v-if="!isWidget && isEntryExtensionEnabled && dataSource !== DataSources.WIKIPEDIA && !extendedViewMode"
				...
				:title="isSignedIn ? $t('popupContent.improveTheResourceDefinition') : $t('popupContent.youNeedToBeSignedInToImprove')"
...
export default class ViewsAndRatingsComponent extends Vue {
	...
	public async entryExtension() {
		if (this.isSignedIn) {
			...
			this.$store.dispatch(PopupActions.openPopup, {contentId: 'entry-extension', title: this.$t('popupTitles.improveDefinition'), component: EntryExtension, props});
		}
	}
}
</script>
