...
export default {
    ...
    methods: {
        parseQueryParams() {
            ...
            if (sessionStorage.getItem('RECENT_SEARCH_PARAMS') && window.location.search === '') {
                const recentSearchParams = UtilsService.getUrlParams(sessionStorage.getItem('RECENT_SEARCH_PARAMS') as string);
                params = recentSearchParams.map((item, i) => Object.assign({}, item, params[i]));
            ...
            return new Promise(resolve => {
                ...
                if (relationType) {
                    this.$store.dispatch(SearchActions.setCrRelationParams, {
                        relationType,
                        crRelationSrcId: UtilsService.getUrlParam('crRelationSrcId'),
                        crRelationSrcDs: UtilsService.getUrlParam('crRelationSrcDs'),
                        workflowInstanceId: UtilsService.getUrlParam('workflowInstanceId'),
                        endpoint: UtilsService.getUrlParam('endpoint'),
                        getPendingRelations: !!UtilsService.getUrlParam('getPendingRelations')
                    });
                    ...
                    if (this.$store.getters[SearchGetters.crRelationParams].crRelationSrcId &&
                        this.$store.getters[SearchGetters.crRelationParams].crRelationSrcDs) {
                        GeossSearchApiService.getTargetById(this.$store.getters[SearchGetters.crRelationParams].crRelationSrcId, this.$store.getters[SearchGetters.crRelationParams].crRelationSrcDs)
                            .then(results => {
                                if (results && results.entry) {
                                    const result = results.entry[0];
                                    const dataOrigin = UtilsService.getUrlParam('crRelationSrcDs');
                                    let dataSource = dataOrigin;
                                    for (const source of Object.keys(DataOrigin)) {
                                        if (source === dataOrigin) {
                                            if (dataOrigin === 'geoss_cr') {
                                                const hub = result.category.filter((el: { _attributes: { term: string; }; }) => el._attributes.term === 'geoss_cr')[0]._attributes.label.split('_')[0];
                                                dataSource = hub === 'service' ? 'services' : hub;
                                            } else {
                                                dataSource = source;
                                            }
                                        }
                                    ...
                                    if (this.$store.getters[SearchGetters.crRelationParams].getPendingRelations) {
                                        GeossSearchApiService.entryRelationsAPI(true)
                                            .then((pendingRelations: { destEntry: any; srcEntry: any; }) => {

                                                this.$store.dispatch(SearchActions.setCrRelation, {
                                                    relationType,
                                                    srcEntry: result,
                                                    destEntry: pendingRelations.destEntry
                                                });

                                                const targetEntries = [pendingRelations.srcEntry, ...pendingRelations.destEntry];
                                                const targetIdsObject = {};

                                                for (const entry of targetEntries) {
                                                    let source: string | null = entry.dataSource;
                                                    let hub = entry.type.split('_')[0];
                                                    hub = hub === 'service' ? 'services' : hub;

                                                    if (entry.dataSource === 'geoss_cr') {
                                                        source = hub;
                                                    }

                                                    if (source) {
                                                        if (!(targetIdsObject as any)[source]) {
                                                            (targetIdsObject as any)[source] = [];
                                                        }
                                                        (targetIdsObject as any)[source].push(entry.id);
                                                    }
                                                }
                ...
                if (savedSearchId || popularSearchId) {
                    new Promise(resolve => {
                        if (savedSearchId) {
                            GeossSearchApiService.getSavedSearchById(savedSearchId).then(resolve);
                        } else {
                            GeossSearchApiService.getPopularSearchById(popularSearchId).then(resolve);
                        }
                    }).then((data: any) => {
                        if (data) {
                            const promises = [];
                            if (data.currMap) {
                                this.$store.dispatch(MapActions.setActiveLayerTileId, data.currMap);
                            }
                            if (data.query) {
                                this.$store.dispatch(GeneralFiltersActions.setPhrase, data.query);
                            }
                            if (data.aoiRelation) {
                                this.$store.dispatch(GeneralFiltersActions.setBoundingBoxRelation, data.aoiRelation);
                            }
                            if (data.dateFrom) {
                                this.$store.dispatch(GeneralFiltersActions.setDateFrom, data.dateFrom);
                            }
                            if (data.dateTo) {
                                this.$store.dispatch(GeneralFiltersActions.setDateTo, data.dateTo);
                            }
                            if (data.datePeriod) {
                                this.$store.dispatch(GeneralFiltersActions.setDatePeriod, data.datePeriod);
                            }
                            if (data.aoiOption) {
                                const oldType = ['ContinentAndCountry', 'Coordinates', 'Geolocation'].indexOf(data.aoiOption);
                                if (oldType !== -1) {
                                    data.aoiOption = ['continent_country', 'coordinates', 'geolocation'][oldType];
                                }
                                this.$store.dispatch(GeneralFiltersActions.setLocationType, data.aoiOption);
                            }
                            if (data.folder) {
                            ...
                            if (coordinates && coordinates.split(',').length === 4) {
                                const [S, W, N, E] = coordinates.split(',').map((item: string) => parseFloat(item));
                                promises.push(this.$store.dispatch(GeneralFiltersActions.setSelectedAreaCoordinates, { W, S, E, N }));
                            }
                    ...
                    if (phrase) {
                        this.$store.dispatch(GeneralFiltersActions.setPhrase, phrase);
                    }
                    ...
                    const currentMap = UtilsService.getUrlParam('m:activeLayerTileId');
                    if (currentMap) {
                        this.$store.dispatch(MapActions.setActiveLayerTileId, currentMap);
                    }

                    this.$store.dispatch(SearchActions.setHighlightResult, targetId);
                    resolve(true);

                } else if (layers) {
                    const { outputName, url, name, protocol, id, runId, legend } = JSON.parse(layers);
                    let version = '1.1.1';
                    if (protocol) {
                    ...
                    GeossSearchApiService.getRunCoordinates(runId).then((coordinate: any) => {
                        ...
                        setTimeout(() => {
                            ...
                            const mapLayers = this.$store.getters[MapGetters.layers].filter((layerData: LayerData) => (
                                (layerData.type.indexOf(LayerTypes.BOUNDING) === -1 || layerData.type === LayerTypes.BOUNDING))
                            );
                            const supportedLegendTypes = this.$store.getters[MapGetters.supportedLegendTypes];
                            const recentLayerIndex = mapLayers.length - 1;
                            if (mapLayers[recentLayerIndex].legend && supportedLegendTypes.includes(mapLayers[recentLayerIndex].legend.type)) {
                                this.$store.dispatch(MapActions.setActiveLayerLegend, mapLayers[recentLayerIndex]);
                            }
                    ...
                    for (const param of params) {
                        if (param.name.indexOf('f:') === 0) {
                            ...
                            const storeParamName = cuttedParamName.charAt(0).toUpperCase() + cuttedParamName.substr(1);

                            if ((GeneralFiltersActions as any)['set' + storeParamName]) {
                                if (cuttedParamName === 'sources') {
                                    param.value = [...param.value.split(',')];
                                }

                                if (cuttedParamName === 'selectedAreaCoordinates') {
                                    if (param.value && param.value.split(',').length === 4) {
                                        const [S, W, N, E] = param.value.split(',').map((item: string) => parseFloat(item));
                                        promises.push(this.$store.dispatch(GeneralFiltersActions.setSelectedAreaCoordinates, { W, S, E, N }));
                                    }
                                } else {
                                    const oldType = ['ContinentAndCountry', 'Coordinates', 'Geolocation'].indexOf(param.value);
                                    if (oldType !== -1) {
                                        param.value = ['continent_country', 'coordinates', 'geolocation'][oldType];
                                    }
                                    promises.push(this.$store.dispatch((GeneralFiltersActions as any)['set' + storeParamName], param.value));
                                }
                            } else if ((FacetedFiltersActions as any)['set' + storeParamName]) {
                                promises.push(this.$store.dispatch((FacetedFiltersActions as any)['set' + storeParamName], param.value));
                            } else if ((GranulaFiltersActions as any)['set' + storeParamName]) {
                                ...
                                if (storeParamName === 'ParentRefs') {
                                    ...
                                    if (value[0].dataSource === DataSources.DAB) {
                                        promises.push(this.$store.dispatch(SearchActions.setParentRefs, value));
                                    }
                                } else if (storeParamName === 'StartIndex') {
                                    promises.push(this.$store.dispatch(SearchActions.setStartIndex, { value: param.value, dataSource: this.$store.getters[SearchGetters.dataSource] }));
                                } else if (storeParamName === 'DataSource') {
                                    promises.push(this.$store.dispatch((SearchActions as any)['set' + storeParamName], { value: param.value }));
                                } else {
                                    promises.push(this.$store.dispatch((SearchActions as any)['set' + storeParamName], param.value));
                                }
                            }
                            ...
                            if ((MapActions as any)['set' + storeParamName]) {
                                promises.push(this.$store.dispatch((MapActions as any)['set' + storeParamName], param.value));
                            }
                        }
                    }
