...
const MapCoordinatesUtils = {
	...
	coordinatesForDrawing(coordinates): any {
		const W = coordinates[0];
	...
	coordinatesForPresentation(coordinates) {
		const W = coordinates[0];
		const S = coordinates[1];
		const E = coordinates[2];
		const N = coordinates[3];
	...
	parsePolygon(polygonString) {
		...
		polygonString = polygonString.slice(start + 3, end).split(',');
		const polygonTab = [];
		polygonString.forEach(value => {
	...
	// box order matters, calculation is only one way, box1 is left
	mergeOverlapingBoxes(box1, box2) {
		const W1 = box1[0];
		const E1 = box1[2];
		const W2 = box2[0];
		const E2 = box2[2];
	...
	// box order matters, calculation is only one way, box1 is left
	mergeNonOverlapingBoxes(box1, box2) {
		const S = Math.min(box1[1], box2[1]);
		const N = Math.max(box1[3], box2[3]);
		const W = box1[0];
		const E = box2[2];
	...
	mergeBoxes(boxes) {
		...
		for (const box of boxes) {
			const normalizedLongitude = MapCoordinatesUtils.normalizeLongitude(box[0], box[2]);
			...
			for (let i = 0; i < boxes.length; i++) {
				for (let j = boxes.length - 1; j > i; j--) {
					if (MapCoordinatesUtils.boxesOverlap(boxes[i], boxes[j])) {
						boxes[i] = MapCoordinatesUtils.mergeOverlapingBoxes(boxes[i], boxes[j]);
						boxes.splice(j, 1);
						foundOverlapping = true;
					} else if (MapCoordinatesUtils.boxesOverlap(boxes[j], boxes[i])) {
						boxes[i] = MapCoordinatesUtils.mergeOverlapingBoxes(boxes[j], boxes[i]);
						boxes.splice(j, 1);
						foundOverlapping = true;
					}
				}
		...
		while (boxes.length > 1) {
			...
			for (let i = 0; i < boxes.length; i++) {
				for (let j = boxes.length - 1; j > i; j--) {
					let distance = MapCoordinatesUtils.calcBoxDistance(boxes[i], boxes[j]);
					if (distance <= closestDistance) {
						closestDistance = distance;
						closest1 = i;
						closest2 = j;
					}
					distance = MapCoordinatesUtils.calcBoxDistance(boxes[j], boxes[i]);
					if (distance <= closestDistance) {
						closestDistance = distance;
						closest1 = j;
						closest2 = i;
			...
			boxes[closest1] = MapCoordinatesUtils.mergeNonOverlapingBoxes(boxes[closest1], boxes[closest2]);
