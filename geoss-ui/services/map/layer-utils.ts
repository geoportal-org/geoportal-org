...
const LayersUtils = {
	getIrisCircleLayerData({ coordinates, magnitude, depth }, index: number) {
		const center: any = [
	...
	getPolygonLayerData(coordinates, index: number) {
		const boxFeature = new ol.Feature({
			geometry: new ol.geom.Polygon([MapCoordinatesUtils.dateLineFix(MapCoordinatesUtils.parsePolygon(coordinates))])
		});
	...
	getPinLayerData(coordinates, index: number) {
		const W = [];
		const S = [];
		const E = [];
		const N = [];
	...
	getBoxLayerData(boxes, index: number) {
		const W = [];
		const S = [];
		const E = [];
		const N = [];
		...
		const boxesArray = [];
		for (let i = 0; i < boxes.length; i++) {
			const denormalizedLongitude = MapCoordinatesUtils.denormalizeLongitude(W[i], E[i]);
		...
		if (MapCoordinatesUtils.isWholeWorld(outerBox[0], outerBox[1], outerBox[2], outerBox[3])) {
	...
	setLayerStyle(layer: ol.layer.Layer, mode: number, type: string) {
		let style = null;
		...
		switch (type) {
			...
			case LayerTypes.BOUDNINGPOLYGON: {
				...
				const strokeColorArr = ['rgb(255, 255, 0)', 'transparent', 'rgb(255, 255, 0)', 'transparent'];

				style = LayersUtils.getBoundingBoxStyle(fillColor, strokeColorArr[mode]);
				break;
			}
			...
			case LayerTypes.BOUNDINGCIRCLE: {
				...
				style = LayersUtils.getBoundingCircleStyle(opacityArr[mode], (layer as any).radius, (layer as any).depth);
				break;
			}
		}
	...
	getBoundingCircleStyle(transparency: number, radius: number, depth: number) {
		return new ol.style.Style({
			image: new ol.style.Icon(({
				anchor: [0.5, 0.5],
				opacity: transparency,
	...
	isLayerDisplayed(id) {
		const layerData: LayerData | undefined = AppVueObj.app.$store.getters[MapGetters.layers].find((layerData: LayerData) => layerData.id === id);
		return (layerData && layerData.visible);
	},

	progressBarAddLoading() {
		let progressBarLoading = AppVueObj.app.$store.getters[MapGetters.progressBarLoading];
		if (!progressBarLoading) {
			LayersUtils.progressBarShow();
		}
		AppVueObj.app.$store.dispatch(MapActions.setProgressBarLoading, ++progressBarLoading);
		LayersUtils.progressBarUpdate();
	},

	progressBarAddLoaded() {
		let progressBarLoaded = AppVueObj.app.$store.getters[MapGetters.progressBarLoaded];
		AppVueObj.app.$store.dispatch(MapActions.setProgressBarLoaded, ++progressBarLoaded);
		LayersUtils.progressBarUpdate();
	},

	progressBarUpdate() {
		const progressBarLoading = AppVueObj.app.$store.getters[MapGetters.progressBarLoading];
		const progressBarLoaded = AppVueObj.app.$store.getters[MapGetters.progressBarLoaded];
		const progressBarPercentage = parseFloat((progressBarLoaded / progressBarLoading * 100).toFixed(1));
		AppVueObj.app.$store.dispatch(MapActions.setProgressBarPercentage, progressBarPercentage);
	...
	progressBarHide() {
		...
		if (progressBarLoading === progressBarLoaded) {
			...
			AppVueObj.app.$store.dispatch(MapActions.setProgressBarPercentage, 0);
		}
		AppVueObj.app.$store.dispatch(MapActions.setProgressBarEnable, false);
	},

	async toggleLayer(layer, coordinates, layerImage) {
		const layerData: LayerData | undefined = AppVueObj.app.$store.getters[MapGetters.layers].find((layerData: LayerData) => layerData.id === layer.url);
		if(layerData) {
			AppVueObj.app.$store.dispatch(MapActions.changeLayerVisibility, {id: layer.url, value: !layerData.visible});
		} else {
			if(coordinates) {
				LogService.logElementClick(coordinates, null, layer.id, 'dab', 'Toggle layer', null, '', layer.title);
			}
			let mapLayer = null;
			if (layer.type === LayerTypes.WMS) {
				...
				const [err] = await to(LayersUtils.checkLayerFile(layer.url, 'kml'));
				if(!err) {
					...
					AppVueObj.app.$store.dispatch(PopupActions.openPopup, {contentId: 'error', component: ErrorPopup, props});
				}
			} else if (layer.type === LayerTypes.KMZ) {
				const [err] = await to(LayersUtils.checkLayerFile(layer.url, 'kmz'));
				if(!err) {
			...
			const [, legend] = await to(new Promise(resolve => {
				if(layer.legend) {
			...
			LayersUtils.addOnTileErrorHandler(mapLayer).catch(async () => {
				...
				const props = {
					...
					subtitle: AppVueObj.app.$t('popupContent.mapLayerUnavailable'),
					actions: [
						{event: 'ignore', label: AppVueObj.app.$t('general.ignore')},
						{event: 'disable', label: AppVueObj.app.$t('general.disable')}
					]
				};

				const [, event] = await to(AppVueObj.app.$store.dispatch(PopupActions.openPopup, {contentId: 'error', component: ErrorPopup, props}));
				if(event === 'disable') {
					AppVueObj.app.$store.dispatch(MapActions.removeLayer, layer.url);
				}
			});
			...
			const layerData = {
				...
				title: layer.title,
				coordinate: null,
				image: (layerImage.indexOf('geoss-gray.svg') !== -1 ? null : layerImage),
				legend
			};
			...
			if(coordinates) {
				...
				layerData.coordinate = {W: minLongitude, S: minLatitude, E: maxLongitude, N: maxLatitude};
			}
			...
			if(AppVueObj.app.$store.getters[GeneralGetters.isWidget]) {
				...
				AppVueObj.app.$store.dispatch(MapActions.changeLayerVisibility, {id: LayerTypes.BOUNDING, value: false});

				AppVueObj.app.$store.dispatch(MapActions.setShowFull, true);

				setTimeout(() => {
					AppVueObj.app.$store.dispatch(MapActions.zoomInLayer, layer.url);
				}, (Timers.hideSearchContainer > Timers.closePopup ? Timers.hideSearchContainer : Timers.closePopup));
			}
		}
	},

	reloadTile(layer, event) {
		return new Promise((resolve, reject) => {
			...
			if (tileLoadRetryCount >= tileLoadRetryLimit || layerLoadRetryCount >= layerLoadRetryLimit) {
				reject();
			} else {
				event.tile.load();
				resolve();
	...
	checkLayerFile(urlToResource, type) {
		return GeossSearchApiService.loadLayerFile(urlToResource, type).then(
			data => {
				if (typeof data === 'object') {
					if (!String(data.result).localeCompare('true') && !type.localeCompare('kml')) {
						return Promise.resolve();
					} else if (!type.localeCompare('kml')) {
						if (String(data.resultDetails)) {
							return Promise.reject(
								`Can not add file type kml to map layer, because it is error: ${String(data.resultDetails)}.
					...
					} else if (!String(data.result).localeCompare('true') && !type.localeCompare('kmz')) {
						return Promise.resolve();
					} else if (!type.localeCompare('kmz')) {
						if (String(data.resultDetails)) {
							return Promise.reject(
								`Can not add file type kmz to map layer, because it is error: ${String(data.resultDetails)}.
							  	However, the file can be downloaded <a href="${urlToResource}" target="_blank">here</a>.`
							);
						} else {
							return Promise.reject(
	...
	createStatisticsLayer(title: string, statistics: object, year: string) {
		...
		const statisticsLayer =  new ol.layer.Vector({
			...
			style: (feature: Feature) => {
				...
				if (statistics[layerName]) {
					...
					return new ol.style.Style({
						fill: new ol.style.Fill({
							color: LayersUtils.getColorScale(statistics[layerName], min, max)
						}),
						stroke: new ol.style.Stroke({
							color: 'rgb(0,0,0)',
							width: 1
