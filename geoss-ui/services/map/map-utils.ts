...
const MapUtils = {
	...
	createDragAndDropInteraction: () => {
		MapDragAndDropInteraction.on('addfeatures', async (event: any) => {
			const [, {id, features}] = await to(new Promise(resolve => {
				if (event.file.name.indexOf('.zip') > -1) {
					...
					geojson => {
						const collection = new ol.Collection(new ol.format.GeoJSON().readFeatures(geojson));

						collection.forEach((f: any) => {
							f.getGeometry().transform('EPSG:4326', 'EPSG:3857');
						});

						resolve({id: event.file.name, features: collection});
					});
				} else {
					resolve({id: event.file.name, features: event.features});
				}
			}));
			...
			const layer = new ol.layer.Vector({
				...
				style: (feature: ol.Feature, resolution) => {
					...
					if (featureStyleFunction) {
						...
						return DefaultShapesStyles[feature.getGeometry().getType()];
					}
				}
			});
			layer.setZIndex(8);
	...
	addBoundingBoxInteractions: map => {
		/**
		...
		map.on('pointermove', event => {
			...
			const layer = map.forEachFeatureAtPixel(mouseCoordInMapPixels, (feature, layer) => {
				return layer;
			});

			if (layer) {
				for (const layerData of (AppVueObj.app.$store.getters[MapGetters.boudningLayers] as LayerData[])) {
					if(layerData.value === layer) {
						AppVueObj.app.$store.dispatch(MapActions.setHoveredLayerId, layerData.id);
			...
			if(AppVueObj.app.$store.getters[MapGetters.hoveredLayerId] !== null) {
				AppVueObj.app.$store.dispatch(MapActions.setHoveredLayerId, null);
				AppVueObj.app.$store.dispatch(MapActions.repaintBoudingLayers);
			}
		});
		...
		map.on('click', event => {
			...
			if (layer) {
				const resultIdDetails = AppVueObj.app.$store.getters[SearchGetters.resultIdDetails];
				for (const layerData of (AppVueObj.app.$store.getters[MapGetters.boudningLayers] as LayerData[])) {
					if(layerData.value === layer) {
						AppVueObj.app.$store.dispatch(MapActions.setHoveredLayerId, null);
						AppVueObj.app.$store.dispatch(MapActions.setClickedLayerId, layerData.id);
						if (resultIdDetails !== layerData.id) {
							AppVueObj.app.$store.dispatch(SearchActions.setResultIdDetails, null);
						}
						AppVueObj.app.$store.dispatch(MapActions.repaintBoudingLayers);
						return;
					}
			...
			if(AppVueObj.app.$store.getters[MapGetters.clickedLayerId] !== null) {
				AppVueObj.app.$store.dispatch(MapActions.setHoveredLayerId, null);
				AppVueObj.app.$store.dispatch(MapActions.setClickedLayerId, null);
				AppVueObj.app.$store.dispatch(MapActions.repaintBoudingLayers);
			}
		});
		...
		map.on('click', event => {
			...
			const boundingBoxLayerData: LayerData = allLayers.find(layerData => layerData.id === LayerTypes.BOUNDING);
			if (!boundingBoxLayerData.visible || boundingBoxLayerData.transparency === 0) {
				return;
			}
				...
				for (const layerData of (AppVueObj.app.$store.getters[MapGetters.boudningLayers] as LayerData[])) {
					if(layerData.value === layer) {
						...
						if (layerData.value.getZIndex() !== 20) {
							...
							for (const otherLayerData of (AppVueObj.app.$store.getters[MapGetters.boudningLayers].filter(item => item !== layerData) as LayerData[])) {
								otherLayerData.value.setZIndex(otherLayerData.index + 21);
							}
							layerData.value.setZIndex(20);
						}
					}
	 ...
	 */
	addUNSDStatisticsInteractions: map => {
		map.on('singleclick', event => {
			const hitLayer = event.target.forEachLayerAtPixel(event.pixel, layer => layer, {
				layerFilter: layer => {
					const isCompareLayer = AppVueObj.app.$store.getters[MapGetters.compareLayer];
					const compareBarPosition = AppVueObj.app.$store.getters[MapGetters.compareBarPosition];
					if (isCompareLayer && isCompareLayer.value === layer && event.pixel[0] <= compareBarPosition) {
						return false;
					}
			...
			const popupOverlay = AppVueObj.app.$store.getters[MapGetters.mapTooltip] as ol.Overlay;
			let overlayPosition;
			popupOverlay.setPosition(undefined);

			const UNSDLayers = (AppVueObj.app.$store.getters[MapGetters.customLayers] as LayerData[]);
			if (UNSDLayers.length) {
				const mouseCoordInMapPixels = event.pixel;
				let outmostFeature;
				const layers = [];
				map.forEachFeatureAtPixel(mouseCoordInMapPixels, (feature, layer) => {
					if (layer.get('name') && layer.get('name') === 'UNEP' && layer.getOpacity() && layer === hitLayer) {
						layers.push(layer);
						if (!outmostFeature) {
				...
				if (layers.length && outmostFeature) {
					for (const layer of layers) {
						if (outmostFeature.get('stats_value')) {
							...
							if (desc.length > 60) {
								const truncatedDesc = `${desc.substring(0, 57)}...`;
								desc = truncatedDesc;
							}
							...
							AppVueObj.app.$store.dispatch(MapActions.setMapTooltipMessage, message);
			...
			if (source instanceof TileWMS) {
				...
				const url = source.getGetFeatureInfoUrl(event.coordinate, viewResolution, 'EPSG:3857', {INFO_FORMAT: 'text/html'});
				...
				if(url) {
					getWmsLayerInfo(url)
						.then((response: string) => {
							...
							template.insertAdjacentHTML('afterbegin', response.trim());
							const featureInfoElement = template.querySelector('.featureInfo');
							const data = {};
							if (featureInfoElement) {
								featureInfoElement.querySelectorAll('th').forEach(element => data[element.innerText] = null);
								const keys = Object.keys(data);
								featureInfoElement.querySelectorAll('td').forEach((element, index) => data[keys[index]] = element.innerText);

								keys.forEach(item => {
									const label = AppVueObj.app.$t(`wms.${item.toLowerCase()}`) || item;
									message += `${label}: ${data[item] || '-'}<br>`;
								});
							} else {
						...
						.catch(e => AppVueObj.app.$store.dispatch(MapActions.setMapTooltipMessage, AppVueObj.app.$t('general.errorOccurred')));
				}
			}
		});
	},
	...
	addOnErrorMapSwitcher: () => {
		...
		addsat.getSource().on('tileloaderror', event => {
		...
		mbHybrid.getSource().on('tileloaderror', event => {
	...
	onTileError: () => {
		...
		AppVueObj.app.$store.dispatch(MapActions.setActiveLayerTileId, 'WorldStreetBasemap');
