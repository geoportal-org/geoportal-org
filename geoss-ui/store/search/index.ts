...
const getters: { [key: string]: any } = {
    ...
    filtersParams: (getters: any, rootGetters: any) => {
        ...
        if (params.viewid === 'worldcereal') {
            ...
            if (params.cropConfidence === '0,100') {
                delete params.cropConfidence
            }
            if (params.landCoverConfidence === '0,100') {
                delete params.landCoverConfidence
            }
            if (params.irrigationConfidence === '0,100') {
...
const actions = {
    async getResults(
        ...
        if (!results && err) {
            const errorInfo = {
                title: ($tc('general.backendError') as string).toUpperCase(),
                ...
                switch (currentResults.error) {
                    case 'no_results': {
                        ...
                        if (!otherAvailableSourceResults.length) {
                            errorInfo.title = (
                                $tc('general.noResourcesFound') as string
                            ).toUpperCase()
    ...
    reset({ dispatch }: { dispatch: any }) {
        ...
        dispatch(
            SearchActions.setDataSource,
            {
                value: defaultDS ? defaultDS : DataSources.DAB,
                checkDefault: true
            },
            { root: true }
        )
