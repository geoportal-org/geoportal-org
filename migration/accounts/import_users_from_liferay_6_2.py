...
def create_credentials(record):
    credentials = []
    credential_data = {
        "algorithm": "liferay-pbkdf2",
        "hashIterations": 0
    }
    secret_data = create_secret_data(record)
    credential = {
        "type": "password",
...
def create_secret_data(record):
    liferay_password = record.get('password', '')
    decoded_password = base64.b64decode(liferay_password)
    salt_bytes = decoded_password[8:16]
    salt_encoded = base64.b64encode(salt_bytes)
    password_bytes = decoded_password[16:]
    password_encoded = base64.b64encode(password_bytes)
    return {
        "value": password_encoded.decode("utf-8"),
        "salt": salt_encoded.decode("utf-8")
    }
