...
def main():
    ...
    failed_data_file = os.path.join(failed_data_dir, USERS_SETTINGS_FAILED_RECORDS_FILE)
    ...
    settings_api_url = config.get('personaldata', 'settings_api_url').strip('\'\"')
    data = load_data(data_dir, USERS_SETTINGS_FILE)
    failed_records = process_records(data, keycloak_admin, keycloak_openid, admin_access_token, settings_api_url)

    log_end_time(start_time)

    if failed_records:
        save_failed_records(failed_records, failed_data_dir, USERS_SETTINGS_FAILED_RECORDS_FILE)
