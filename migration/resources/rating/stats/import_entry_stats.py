...
RESOURCE_RATING_STATS_FILE = 'geoss_ResourceRatingStats.json'
RESOURCE_RATING_STATS_FAILED_RECORDS_FILE = 'resource_rating_stats_failed_records.json'
...
def main():
    ...
    failed_data_file = os.path.join(failed_data_dir, RESOURCE_RATING_STATS_FAILED_RECORDS_FILE)
    ...
    stats_api_url = config.get('curated', 'stats_api_url').strip('\'\"')
    data = load_data(data_dir, RESOURCE_RATING_STATS_FILE)
    failed_records = process_records(data, admin_access_token, stats_api_url)

    log_end_time(start_time)

    if failed_records:
        save_failed_records(failed_records, failed_data_dir, RESOURCE_RATING_STATS_FAILED_RECORDS_FILE)
