...
TUTORIAL_TAGS_FAILED_RECORDS_FILE = 'tutorial_tags_failed_records.json'
API_URL = 'https://gpp.devel.esaportal.eu/settings/rest/tags'
...
def create_payload(record):
    ...
    return {
        "name": record.get('id_', ''),
        "show": show,
        "type": record.get('type_', ''),
        "placement": record.get('placement', ''),
        "title": create_title(record),
        "description": create_description(record),
    }


def create_title(record):
    title = ET.fromstring(record.get('title', ''))
    return {
        "en": title.find('./Title[@language-id="en_US"]/.').text,
    }


def create_description(record):
    description = ET.fromstring(record.get('description', ''))
    return {
        "en": description.find('./Description[@language-id="en_US"]/.').text,
    }
